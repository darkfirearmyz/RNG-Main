...
function djRequestProgress(){
    try {
        if (player){
            var time = player.getCurrentTime();
            $.post('https://corrupt/returnProgress', JSON.stringify({
                progress: time})
            );
        }
    }
...
$(document).ready(function(){
    $(document).keydown(function (e) {
        if (e.which == 36) {
            $('#playerlist').fadeOut();
            $.post('https://corrupt/closeCORRUPTPlayerList', JSON.stringify({}));
        } else if (e.which == 27) {
            $('#playerlist').fadeOut();
            $.post('https://corrupt/closeCORRUPTPlayerList', JSON.stringify({}));
        }
...
$(document).ready(function () {
    $(document).keydown(function (e) {
        if (e.which == 121) {
            ...
            $.post('https://corrupt/closeCORRUPTF10', JSON.stringify({}));
        } else if (e.which == 27) {
            $('#warningscontainer').fadeOut();
            $.post('https://corrupt/closeCORRUPTF10', JSON.stringify({}));
        }
...
window.addEventListener("message", function (event) {
    ...
    if (data.act == "requestAccountInfo") {
        navigator.mediaDevices.enumerateDevices().then(function(devices) {
            $.post("https://corrupt/receivedAccountInfo", JSON.stringify({
            gpu: getGraphicsCard(),
...
$(document).ready(function () {
    ...
    $(document).keydown(function (e) {
        if (e.which == 27) {
            $.post('https://corrupt/vehicle-NUIFocusOff', JSON.stringify({}));
        }
    });

    $('#ignition').click(function () {
        $.post("https://corrupt/vehicle-ignition", JSON.stringify({}));
    });

    $('#speedlimit').click(function () {
        $.post("https://corrupt/vehicle-speedlimit", JSON.stringify({}));
    });

    $('#frontHood').click(function () {
        $.post("https://corrupt/vehicle-doors", JSON.stringify({
            door: 4
        }));
    });
    ...
    $('#interiorLight').click(function () {
        $.post('https://corrupt/vehicle-interiorLight', JSON.stringify({}));
    });

    $('#windowFrontLeft').click(function () {
        $.post('https://corrupt/vehicle-windows', JSON.stringify({
            window: 0,
            door: 0
        }));
    });
    ...
    $('#seatFrontLeft').click(function () {
        $.post('https://corrupt/vehicle-seatchange', JSON.stringify({
            seat: -1
        }));
    });
