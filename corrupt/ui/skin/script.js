$(document).ready(function(){
  ...
  $('input').change(function(){
    ...
    $.post('https://corrupt/updateSkin', JSON.stringify({
      value: false,
      // Face
  ...
  $(document).keypress(function(e) {
    ...
    if(e.which == 113){ // A pressed
      $.post('https://corrupt/rotaterightheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 101){ // E pressed
      $.post('https://corrupt/rotateleftheading', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 119 ){ // W pressed
      $.post('https://corrupt/zoomin', JSON.stringify({
        value: 10
      }));
    }
    if(e.which == 115){ // S pressed
      $.post('https://corrupt/zoomout', JSON.stringify({
        value: 10
      }));
    }
  });
  ...
  $('#skin-tabs skin-label').on('click', function(e){
    ...
    $.post('https://corrupt/zoom', JSON.stringify({
...
function leftWristComponent() {
    $.post('https://corrupt/updateLeftWristComponent', JSON.stringify({
...
function leftWristTexture() {
    ...
    $.post('https://corrupt/updateLeftWristTexture', JSON.stringify({
        componentID: $('.montre .active').attr('data'),
...
function rightWristComponent() {
    $.post('https://corrupt/updateRightWristComponent', JSON.stringify({
...
function rightWristTexture() {
    ...
    $.post('https://corrupt/updateRightWristTexture', JSON.stringify({
        componentID: $('.montre-right .active').attr('data'),
