...
async function RegisterVueComponent(component, path) {
    const response = await fetch("nui://corrupt/ui/pnc/components/"+path);
        ...
        methods: {
            addMarker() {
                if(!this.doesPlayerHaveWarningMarkerOfType(this.selectedWarningMarkerType)) {
                    $.post('https://corrupt/addmarker', JSON.stringify({
            ...
            wipeAllMarkers() {
                $.post('https://corrupt/wipeallmarkers', JSON.stringify({
            ...
            addPoint() {
                $.post('https://corrupt/addpoint', JSON.stringify({
            ...
            switchPage(page) {
                if(this.personPages.includes(page)){
                    if (this.selectedPerson.id){
                        ...
                        console.log("[PNC] Person needs to be selected to enter this page")
                    }
                }else {
                    this.page = page;
                }
            ...
            closePNC() {
                $.post('https://corrupt/exit', JSON.stringify({}));
            },
            searchPerson(){
                let first = this.personFirstName;
                let last = this.personLastName;
                if (first == "" || last == "") {
                    console.log("[PNC] Full name needs to be provided")
                }else{
                    $.post('https://corrupt/personsearch', JSON.stringify({
                        firstname: first,
                        lastname: last
                    }));
                }
            ...
            searchPlate(){
                ...
                $.post("https://corrupt/platesearch", JSON.stringify({
                    plate: this.plate
                }))
            },
            selectPerson(person){
            ...
            submitFine() {
                ...
                $.post('https://corrupt/submitfine', JSON.stringify({
            ...
            addAD(){
                ...
                if (this.ADInput.length === 0) {
                    ...
                    $.post('https://corrupt/addattentiondrawn', JSON.stringify({
                        ad: this.ADInput
                    }));
                }
            ...
            removeAD(index, ad) {
                if(this.askedIfSure) {
                    ...
                    $.post('https://corrupt/removeattentiondrawn', JSON.stringify({
            ...
            saveNotes() {
                $.post('https://corrupt/savenotes', JSON.stringify({
            ...
            getPlateImage(plate) {
                $.get(`https://api.arthur.gg/image/plate?plate=${plate}`)
                return `https://api.arthur.gg/image/plate?plate=${plate}`
            },
            getTaxDisc(name, plate, id) {
                $.get(`https://api.arthur.gg/image/tax?plate=${plate}&name=${name}&id=${id}`)
                return `https://api.arthur.gg/image/tax?plate=${plate}&name=${name}&id=${id}`
            },
            savePersonNotes() {
                $.post("https://corrupt/savepersonnotes", JSON.stringify({
                    user_id: this.selectedPerson.id,
                    notes: this.selectedPerson.notes
                }))
            },
            GetWarrant() {
                $.post("https://corrupt/generatewarrant", JSON.stringify({}))
            }
            },
