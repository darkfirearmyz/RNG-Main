window.addEventListener("load", function() {
    ...
    wprompt.onClose = function() {
        $.post("https://corrupt/prompt", JSON.stringify({ act: "close", result: wprompt.result.substring(0, 1000) }));
    };

    requestmgr.onResponse = function(id, ok) {
        $.post("https://corrupt/request", JSON.stringify({ act: "response", id: id, ok: ok }));
    };
