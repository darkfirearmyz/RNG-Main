...
--         TriggerEvent("CORRUPT:serverIssue", string.gsub(trace, ":(.*)", ""), trace, variables, false)
--     else
--         TriggerServerEvent("CORRUPT:clientIssue", string.gsub(trace, ":(.*)", ""), trace, variables, false)
--     end
-- end
...
-- function CORRUPT.logError(title, body, data)
--     if data then
--         data = "```" .. data .. "```"
--     end
--     if IsDuplicityVersion() then
--         TriggerEvent("CORRUPT:serverIssue", title, body, data, true)
--     else
--         TriggerServerEvent("CORRUPT:clientIssue", title, body, data, true)
--     end
-- end

function CORRUPT.isHalloween()
    return GetConvarInt("corrupt_halloween", 0) == 1
end
function CORRUPT.isChristmas()
    return GetConvarInt("corrupt_christmas", 0) == 1
end