...
RMenu.Add("corruptevents","main",
    RageUI.CreateMenu("", "Event Menu", CORRUPT.getRageUIMenuWidth(), CORRUPT.getRageUIMenuHeight(), "corrupt_events", "corrupt_events")
)
RMenu.Add(
    "corruptevents",
    "categoryInfo",
    RageUI.CreateSubMenu(
        RMenu:Get("corruptevents", "main"),
        "",
        "Category",
...
RMenu.Add(
    "corruptevents",
    "players",
    RageUI.CreateMenu("", "Players", CORRUPT.getRageUIMenuWidth(), CORRUPT.getRageUIMenuHeight(), "corrupt_events", "corrupt_events")
)
RMenu.Add(
    "corruptevents",
    "players2",
...
RMenu.Add(
    "corruptevents",
    "client",
    RageUI.CreateMenu("", "CORRUPT Events", CORRUPT.getRageUIMenuWidth(), CORRUPT.getRageUIMenuHeight(), "corrupt_events", "corrupt_events")
)
local function g()
    return CORRUPT.isDeveloper(CORRUPT.getUserId()) or CORRUPT.getStaffLevel() >= 6
end
RegisterCommand("eventmenu",function()
    if g() then
        if currentEvent.isManager == true then
            RageUI.CloseAll()
            RageUI.Visible(RMenu:Get("corruptevents", "players"), true)
        else
            if RageUI.Visible(RMenu:Get("corruptevents", "main")) then
                RageUI.CloseAll()
            else
                RageUI.CloseAll()
                RageUI.Visible(RMenu:Get("corruptevents", "main"), true)
            end
        end
    end
end,false)
local function h(i)
    RageUI.ButtonWithStyle(
        ...
        function(j, k, l)
            if l then
                RMenu:Get("corruptevents", "secondary").MetaData = i
            end
        end,
        RMenu:Get("corruptevents", "secondary")
    )
end
RageUI.CreateWhile(
    1.0,
    ...
    function()
        RageUI.IsVisible(
            ...
            function()
                if GetGameTimer() - e > 1000 then
                    TriggerServerEvent("CORRUPT:requestIsAnyEventActive")
                    e = GetGameTimer()
                end
                if f then
                ...
                for m, n in pairs(a) do
                    if table.count(n) > 1 then
                        RageUI.ButtonWithStyle(
                            ...
                            RMenu:Get("corruptevents", "categoryInfo")
                        )
                    else
        ...
        RageUI.IsVisible(
            RMenu:Get("corruptevents", "players"),
            true,
            true,
            true,
            function(p)
                ...
                RageUI.ButtonWithStyle(
                    ...
                    function(j, k, l)
                        if l then
                            TriggerServerEvent("CORRUPT:startEventFully", currentEvent.eventId)
                            RageUI.CloseAll()
                        end
                    end
                )
                RageUI.ButtonWithStyle(
                    ...
                    function(j, k, l)
                        if l then
                            TriggerServerEvent("CORRUPT:cancelEvent", currentEvent.eventId)
                            RageUI.CloseAll()
                        end
                    end
                )
                ...
                for t, u in pairs(currentEvent.players) do
                    RageUI.ButtonWithStyle(
                        ...
                        function(j, k, l)
                            if l then
                                RMenu:Get("corruptevents", "players2").MetaData = u
                            end
                        end,
                        RMenu:Get("corruptevents", "players2")
                    )
                end
            end
        )
        RageUI.IsVisible(
            ...
            function(p)
                RageUI.ButtonWithStyle(
                    ...
                    function(j, k, l)
                        if l then
                            TriggerServerEvent("CORRUPT:kickPlayerFromEvent", p.source, currentEvent.eventId)
                        end
                    end
                )
            end
        ...
        RageUI.IsVisible(
            RMenu:Get("corruptevents", "client"),
            true,
            true,
            true,
            function(p)
...
function CORRUPT.registerEventMenuItem(v, w, x)
    if not a[v] then
        a[v] = {}
    end
    a[v][w] = {name = w, menuitems = x}
end
function CORRUPT.registerEventFullStartHandler(y, z)
    b.start[y] = z
end
function CORRUPT.registerEventCleanupHandler(y, z)
    b.cleanup[y] = z
end
local function A(B)
    tvRP.removeBlipsForPlayer(B)
    CORRUPT.removePlayerFromLeaderboard(B.source)
end
RegisterNetEvent(
    "CORRUPT:syncPlayers",
    function(C, D)
...
RegisterNetEvent(
    "CORRUPT:addEventPlayer",
    function(B)
        table.add(currentEvent.players, B)
    end)
RegisterNetEvent(
    "CORRUPT:removeEventPlayer",
    function(B)
...
RegisterNetEvent("CORRUPT:startHostEventMenu",function()
...
function CORRUPT.stopEventSequence(M)
    RageUI.CloseAll()
    isDoingSequence = false
    DestroyCam(I, false)
    DestroyCam(J, false)
...
RegisterNetEvent(
    "CORRUPT:startEventFully",
    function(p)
        globalDisableVehicleFailure = true
        currentEvent.data = p
        currentEvent.isActive = true
        CORRUPT.setGreenzonesDisabled(true)
        RageUI.Visible(RMenu:Get("corruptevents", "client"), false)
    end
)
local N = "..."
    ...
    function()
        while true do
            if isDoingSequence then
                ...
                if not RageUI.Visible(RMenu:Get("corruptevents", "client")) and not currentEvent.isManager then
                    RageUI.Visible(RMenu:Get("corruptevents", "client"), true)
                elseif not RageUI.Visible(RMenu:Get("corruptevents", "players")) and currentEvent.isManager then
                    RageUI.Visible(RMenu:Get("corruptevents", "players"), true)
                end
            end
...
RegisterNetEvent(
    "CORRUPT:eventCleanup",
    function(D, P, Q, R)
        CORRUPT.stopEventSequence()
        RageUI.Visible(RMenu:Get("corruptevents", "client"), false)
        if b.cleanup[P] then
            b.cleanup[P]()
        end
        globalDisableVehicleFailure = false
        CORRUPT.setEventSpectatorMode(false)
        CORRUPT.clearEventBounds()
        CORRUPT.enableEventPlayerBlips(false)
        CORRUPT.enableEventPlayerTags(false, false)
        CORRUPT.setGreenzonesDisabled(false)
        CORRUPT.setPlayerCanOpenLeaderboard(false)
        CORRUPT.clearLeaderboardData()
        if Q and R then
            CORRUPT.podiumLeaderboard(Q, R)
        elseif CORRUPT.isPodiumDrawing() then
            CORRUPT.callCancelPodium()
        end
        currentEvent = {
            players = {},
            isActive = false,
...
function CORRUPT.setPlayerInvisible(S)
    local T = PlayerPedId()
...
RegisterNetEvent(
    "CORRUPT:announceEventJoinable",
    function(P, U)
        if tvRP.getPlayerCombatTimer() == 0 and not CORRUPT.getHideEventAnnouncementFlag() and not CORRUPT.isInTutorial() then
            PlaySound(-1, "CHECKPOINT_PERFECT", "HUD_MINI_GAME_SOUNDSET", false, 0, true)
            CORRUPT.announceMpBigMsg("~b~" .. P .. " event has started!", "/joinevent to enter, Win £500,000! - " .. tostring(U) .. " slots available.", 15000)
        else
            TriggerEvent('chatMessage', "^7[EVENT] | ^1" .. P .."^7 : " , { 128, 128, 128 }, "An event has started! Use /joinevent to enter, Win £500,000! - " .. tostring(U) .. " slots available.", "alert", "OOC")
        end
    end
)
function CORRUPT.announceMpBigMsg(V, W, X, Y, Z)
    if Y then
        local _ = GetSoundId()
...
RegisterNetEvent("CORRUPT:announceMpBigMsg", CORRUPT.announceMpBigMsg)
function CORRUPT.setEventMusic(a2)
    currentEvent.playMusic = true
    currentEvent.musicString = a2
    TriggerMusicEvent(a2)
end
function CORRUPT.getActiveEventPlayers()
    local a3 = {}
    for o, B in pairs(currentEvent.players) do
        if B.active then
            table.insert(a3, B)
...
function CORRUPT.getEventLocalPlayer()
...
function CORRUPT.getEventPlayerFromSrc(a6)
    for o, a5 in pairs(currentEvent.players) do
...
function CORRUPT.showCountdownTimer(a9)
...
function CORRUPT.setEventBounds(ag)
    if ag and table.count(ag) ~= 0 then
        currentEvent.bounds = ag
    end
end
function CORRUPT.clearEventBounds()
...
Citizen.CreateThread(
    function()
        while true do
            if
                currentEvent.bounds and CORRUPT.getEventLocalPlayer() and CORRUPT.getEventLocalPlayer().active and
...
local function an(ao)
    CORRUPT.loadPtfx("proj_indep_firework")
    CORRUPT.loadPtfx("proj_indep_firework_v2")
    ...
    CORRUPT.loadPtfx("scr_indep_fireworks")
...
local as = false
function CORRUPT.isPodiumDrawing()
    return as
end
local at = false
function CORRUPT.callCancelPodium()
    at = true
end
function CORRUPT.podiumLeaderboard(Q, R)
    as = true
    CORRUPT.setTime(0, 0, 0)
    RequestIpl("stadium")
...
RegisterNetEvent(
    "CORRUPT:celebrationScreen",
    function(aE, aF)
        CreateThread(
            function()
                CORRUPT.activateSlowMo()
            end
        )
        local aG = {}
        ...
        if not CORRUPT.isPodiumDrawing() then
            CORRUPT.showUI()
        end
    end
)
local aI = false
function CORRUPT.activateSlowMo()
    aI = true
...
local aM
local aN = false
function CORRUPT.enableEventPlayerBlips(aO, aP, aQ)
    aL = aP
    aM = aQ or function()
            return true
        end
...
local aR = {}
function CORRUPT.forceCleanupPlayerBlips()
...
function tvRP.removeBlipsForPlayer(B)
    if B.blip then
        RemoveBlip(B.blip)
    end
                    ...
                    if aX ~= -1 and aX ~= PlayerId() and table.has(a3, aX) then
                        ...
                        if IsEntityVisible(T) and a5.active and aM(a5.source) then
                            if not a5.blip or aW ~= a5.blip then
                                ...
                                AddTextComponentSubstringPlayerName(CORRUPT.getPlayerName(aX))
...
local b2 = false
local b3 = false
function CORRUPT.enableEventPlayerTags(aO, b4, b5, b6)
    b0 = b5
    b1 = b6
    b3 = b4
    b2 = aO
...
function CORRUPT.isEventPlayerTagEnabled()
    return b2
end
CreateThread(
    function()
            ...
            if b2 then
                ...
                for r, a5 in ipairs(currentEvent.players) do
                    ...
                    if aX ~= -1 and aX ~= PlayerId() then
                        ...
                        local b8 = HasEntityClearLosToEntity(T, b7, 17) or CORRUPT.isSpectatingEvent()
                        if b8 and IsEntityVisible(b7) and a5.active then
                            if not a5.tag or not IsMpGamerTagActive(a5.tag) then
                                ...
                                if b1 then
                                    ...
                                    b9 = CreateFakeMpGamerTag(b7, CORRUPT.getPlayerName(aX), false, false, "", 0)
                                end
                                if b0 then
...
function CORRUPT.setEventIntroMessage(ba, bb, bc, bd)
    EnableAllControlActions(0)
    local _ = GetSoundId()
    PlaySoundFrontend(_, "Frontend_Beast_Freeze_Screen", "FM_Events_Sasquatch_Sounds", false)
    ReleaseSoundId(_)
    if not HasStreamedTextureDictLoaded("corruptui") then
        RequestStreamedTextureDict("corruptui", false)
        while not HasStreamedTextureDictLoaded("corruptui") do
            Wait(0)
        end
    end
    CORRUPT.hideUI()
    ...
    if bc == 0 then
        ...
        while bg do
            ...
            DrawSprite("corruptui", "slanted_rect", 0.5, be, 1.0, 1.0, 0.0, 255, 255, 255, 180)
        end
    end
    CORRUPT.showUI()
end
function CORRUPT.inEvent()
    return currentEvent.isActive or isDoingSequence or d
end
RegisterNetEvent(
    "CORRUPT:setIsAnyEventActive",
    function(a8)
        f = a8
    end
)
RegisterCommand("joinevent", function()
    if tvRP.isInComa() then
        tvRP.notify("~r~Your unable to join an event while dead.")
        return
    elseif CORRUPT.inEvent() then
        tvRP.notify("~r~You are already in an event.")
        return
    elseif tvRP.getPlayerCombatTimer() > 0 then
        tvRP.notify("~r~You cannot join an event while in combat.")
        return
    end
    TriggerServerEvent("CORRUPT:joinEventServer")
end)


RegisterNetEvent("CORRUPT:tpcoordsevent", function(x, y, z)
