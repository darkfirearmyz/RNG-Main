function CORRUPT.add3DTextForCoord(a, b, c, d, e)
    local function f(g)
        CORRUPT.DrawText3D(g.coords, g.text, 0.2)
    end
    local h = generateUUID("3dtext", 8, "alphanumeric")
    CORRUPT.createArea(
        "3dtext_" .. h,
        vector3(b, c, d),
        e,
        ...
        f,
        {coords = vector3(b, c, d), text = a}
    )
end
function CORRUPT.drawFloatingHelpText(a, i)
...
function CORRUPT.DrawText3D(i, a, j, k, l, m)
...
function CORRUPT.DrawText(b, c, a, j, k, p, m, q)
    SetTextScale(j, j)
    SetTextFont(k or 0)
    if p then
