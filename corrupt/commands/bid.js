...
exports.runcmd = (fivemexports, client, message, params) => {
    ...
    fivemexports.corrupt.execute("SELECT user_id FROM `corrupt_verification` WHERE discord_id = ?", [user.id], (result) => {
        if (result.length > 0) {
            fivemexports.corrupt.execute("SELECT * FROM corrupt_user_moneys WHERE user_id = ?", [result[0].user_id], (result) => {
