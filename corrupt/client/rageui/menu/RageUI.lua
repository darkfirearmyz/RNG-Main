...
function RageUI.IsVisible(menu, header, glare, instructional, items, panels)
    if (RageUI.Visible(menu)) then
        if (header == true) then
            pcall(RageUI.Banner,true, false)
        end
...
function RageUI.CreateWhile(wait, menu, key, closure)
    -- if not closure then
    --     print(json.encode(menu), key, closure)
    --     return
    -- else
    --     --print(tostring(closure), json.encode(menu))
    -- end
...
function CORRUPT.getRageUIMenuWidth()
...
function CORRUPT.getRageUIMenuHeight()
    return 100
end