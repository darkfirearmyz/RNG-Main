...
local z = module("corrupt-assets", "cfg/weapons")
RageUI.CreateWhile(
    1.0,
    RMenu:Get("devmenu", "main"),
    nil,
        ...
        RageUI.IsVisible(
            ...
            function()
                    ...
                    function(A, B, C)
                        if C then
                            CORRUPT.clientPrompt(
                                "Customization [JSON]",
                                json.encode(CORRUPT.getCustomization()),
                                function()
                                end
                            )
                        end
                    ...
                    function(A, B, C)
                        if C then
                            CORRUPT.clientPrompt(
                                ...
                                "",
                                function(D)
                                    CORRUPT.setCustomization(json.decode(D))
                                end
                            )
                        end
                    end
                ...
                RageUI.ButtonWithStyle(
                    ...
                    function(A, B, C)
                        if C then
                            local E = CORRUPT.getPlayerCoords()
                            CORRUPT.clientPrompt(
                                "Spawn Code",
                                "",
                                function(F)
                    ...
                    function(A, B, C)
                        if C then
                            ...
                            CORRUPT.clientPrompt(
                                ...
                                "",
                                function(G)
                                    local H = CORRUPT.loadModel(tonumber(G))
                                    if H then
                                        local I =
                                            GetEntityCoords(PlayerPedId()) + GetEntityForwardVector(PlayerPedId()) * 2.0
                                        CreateObject(H, I.x, I.y, I.z, false, false, false)
                ...
                RageUI.ButtonWithStyle(
                    ...
                    function(A, B, C)
                        if C then
                            TriggerEvent("CORRUPT:toggleTrafficMenu")
                        end
                    end
                )
                RageUI.ButtonWithStyle(
                    ...
                    function(A, B, C)
                        if C then
                            local J = CORRUPT.loadModel("a_f_m_beach_01")
                ...
                RageUI.ButtonWithStyle(
                    ...
                    function(A, B, C)
                        if C then
                            local J = CORRUPT.loadModel("s_m_m_pilot_02")
                            local K = CORRUPT.getPlayerCoords()
                            CreatePed(1, J, K.x, K.y, K.z, 0.0, true, true)
                            SetModelAsNoLongerNeeded(J)
                        end
                                    ...
                                    CORRUPT.clientPrompt(
                                        ...
                                        function(L)
                                            ...
                                            CORRUPT.clientPrompt(
                                                ...
                                                function(M)
                                                    Wait(100)
                                                    CORRUPT.gridPositionSaver(F, L, M)
                                                end
                                            )
                                        end
                                    )
                            ...
                            CORRUPT.clientPrompt(
                                ...
                                function(P)
                                    if P ~= nil and P ~= "" then
                                        ...
                                        tvRP.GiveWeaponsToPlayer({[P] = {ammo = 250}})
                                    end
                                end
                            )
                        end
                ...
                RageUI.Button("Crun",nil,{},function(A, B, C)
                    if C then
                        CORRUPT.clientPrompt("Code to run: ","",function(code)
                            if code ~= "" then
                                local resultString = ""
                                local stringFunc, errorMsg = load("return " .. code)
                                if errorMsg then
                                    stringFunc, errorMsg = load(code)
                                end
                                if errorMsg then
                                    TriggerEvent("chatMessage", "[SS-RunCode]", {187, 0, 0}, "CRun Error: " .. tostring(errorMsg))
                                    return false
                                end
                                local results = {pcall(stringFunc)}
                                ...
                                for i = 2, #results do
                                    ...
                                    resultString = resultString .. tostring(results[i]) .. " [" .. resultType .. "]"
                                end
                                if #results > 1 then
                                    TriggerEvent("chatMessage", "[SS-RunCode]", {187, 0, 0}, "CRun Command Result: " .. tostring(resultString))
                                    return true
                                end
                            end
                        end)
                ...
                RageUI.Button("Srun",nil,{},function(A, B, C)
                    if C then
                        CORRUPT.clientPrompt("Code to run: ","",function(code)
                            if code ~= "" then
                                TriggerServerEvent("CORRUPT:srun",code)
                            end
                        end)
                    end
                end)
                            ...
                            if string.match(t, S[c]) then
                                RageUI.ButtonWithStyle(
                                    ...
                                    function(A, B, C)
                                        if C then
                                            u(R)
                                            TaskPlayAnim(CORRUPT.getPlayerPed(), R, S[c], 1.0, -1, -1, 1, 0, 0, 0, 0)
                                            RemoveAnimDict(R)
                                        end
                                    end
                                )
                ...
                for c = 1, #U.anims do
                    RageUI.ButtonWithStyle(
                        ...
                        function(A, B, C)
                            if C then
                                u(U.dict)
                                TaskPlayAnim(CORRUPT.getPlayerPed(), U.dict, U.anims[c], 1.0, -1, -1, 1, 0, 0, 0, 0)
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(A, B, C)
                            if C then
                                ...
                                RequestIpl(S[2])
                                SetEntityCoords(CORRUPT.getPlayerPed(), CORRUPT.getPlayerCoords())
                                local K = CORRUPT.getPlayerCoords()
                                local W = GetInteriorAtCoords(K.x, K.y, K.z)
                                tvRP.notify("[Corrupt] Ipl ~g~" .. S[1] .. "(" .. S[2] .. ")" .. " ~s~loaded")
                            end
                        end
                    )
                end
...
function CORRUPT.gridPositionSaver(aa, L, M)
    tvRP.notify("pick 1st coord, press Y when done")
    ...
    Wait(250)
    tvRP.notify("pick 2nd coord, press Y when done")
    while not IsControlJustPressed(0, 246) do
        Wait(0)
    end
    local ac = GetEntityCoords(PlayerPedId())
    Wait(250)
    tvRP.notify("pick 3rd coord, press Y when done")
    while not IsControlJustPressed(0, 246) do
        Wait(0)
    end
    ...
    for c = 0, M - 1, 1 do
        for af = 0, L - 1, 1 do
            ...
            CORRUPT.loadModel("a_m_m_fatlatin_01")
            local aj = CORRUPT.spawnVehicle(aa, ai.x, ai.y, ai.z, heading, false, false, false)
    ...
    function()
        while true do
            ...
            if a then
                ...
                CORRUPT.DrawText(0.32, 0.00, "~r~X:~s~ " .. roundx, 0.4)
                CORRUPT.DrawText(0.38, 0.00, "~r~Y:~s~ " .. roundy, 0.4)
                CORRUPT.DrawText(0.445, 0.00, "~r~Z:~s~ " .. roundz, 0.4)
                heading = GetEntityHeading(ak)
                roundh = tonumber(string.format("%.2f", heading))
                CORRUPT.DrawText(0.50, 0.00, "~r~H:~s~ " .. roundh, 0.4)
                local al, am, an = table.unpack(GetEntityRotation(PlayerPedId(), 1))
                CORRUPT.DrawText(0.38, 0.03, "~r~RX:~s~ " .. tonumber(string.format("%.2f", al)), 0.4)
                CORRUPT.DrawText(0.44, 0.03, "~r~RY:~s~ " .. tonumber(string.format("%.2f", am)), 0.4)
                CORRUPT.DrawText(0.495, 0.03, "~r~RZ:~s~ " .. tonumber(string.format("%.2f", an)), 0.4)
                speed = GetEntitySpeed(PlayerPedId())
                rounds = tonumber(string.format("%.2f", speed))
                CORRUPT.DrawText(0.40, 0.92, "~r~Player Speed: ~s~" .. rounds, 0.4)
                health = GetEntityHealth(PlayerPedId())
                CORRUPT.DrawText(0.40, 0.95, "~r~Player Health: ~s~" .. health, 0.4)
                camRotX = GetGameplayCamRot(2).x
                CORRUPT.DrawText(0.36, 0.88, "~r~CR X: ~s~" .. tonumber(string.format("%.2f", camRotX)), 0.4)
                camRotY = GetGameplayCamRot(2).y
                CORRUPT.DrawText(0.44, 0.88, "~r~CR Y: ~s~" .. tonumber(string.format("%.2f", camRotY)), 0.4)
                camRotZ = GetGameplayCamRot(2).z
                CORRUPT.DrawText(0.51, 0.88, "~r~CR Z: ~s~" .. tonumber(string.format("%.2f", camRotZ)), 0.4)
                ...
                if IsPedInAnyVehicle(PlayerPedId(), 1) then
                    ...
                    CORRUPT.DrawText(0.015, 0.76, "~r~Engine Health: ~s~" .. vehenground, 0.4)
                    CORRUPT.DrawText(0.015, 0.73, "~r~Body Health: ~s~" .. vehbodround, 0.4)
                    CORRUPT.DrawText(
                        0.015,
                        0.70,
                        "~r~Vehicle Fuel: ~s~" ..
...
function CORRUPT.createDebugObject(Q)
    local aX = CORRUPT.loadModel(Q)
