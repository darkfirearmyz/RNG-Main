...
local function r()
    return CORRUPT.getStaffLevel() >= 6 or CORRUPT.isDeveloper()
end
RegisterNetEvent(
    "CORRUPT:spectatePlayer",
    function(s, t)
        if r() then
            m = 0
        end
...
RegisterCommand(
    ...
    function(s, t)
        if r() then
            ...
            if u then
                TriggerServerEvent("CORRUPT:spectatePlayer", u)
            end
        end
    end
)
...
RegisterNetEvent(
    ...
    function()
        ...
        if #(CORRUPT.getPlayerCoords() - g) > 2142.0 then
            if CORRUPT.isInCayoPerico() then
...
local function z(A, B, C, D, E, F, G, H, I, J)
    CORRUPT.DrawText(A - C / 2, B - D / 2 + 0.005, F, E, 0, 1, {G, H, I, J})
end
local function K(L)
...
local function aI(aJ)
    ...
    if IsControlJustPressed(0, 121) then
        TriggerServerEvent("CORRUPT:checkEnhancedPerms")
        if m == 0 then
            m = 1
        elseif m == 1 then
            m = 2
                ...
                if aP ~= -1 then
                    ...
                    if aJ ~= 0 then
                        ...
                        if #(GetFinalRenderedCamCoord() - g) < 2142.0 then
                            if not CORRUPT.isInCayoPerico() then
                    ...
                    tvRP.notify("~r~Couldn't spectate, person not in your zone")
                end
            end
        end
    end
...
CORRUPT.createThreadOnTick(b7)
function CORRUPT.isInSpectate()
    return a
end
