...
RegisterNetEvent(
    "CORRUPT:clearVehicles",
    function()
        for b, c in pairs(GetGamePool("CVehicle")) do
            if NetworkHasControlOfEntity(c) then
                if GetPedInVehicleSeat(c, -1) == 0 and c ~= GetVehiclePedIsIn(CORRUPT.getPlayerPed(), true) then
                    if a[GetEntityModel(c)] == nil and GetEntityModel(c) ~= "casgt86" then
                        DeleteEntity(c)
                    end
                end
            end
...
RegisterNetEvent("CORRUPT:clearBrokenVehicles",function()
        for b, c in pairs(GetGamePool("CVehicle")) do
            if NetworkHasControlOfEntity(c) then
                if GetPedInVehicleSeat(c, -1) == 0 and c ~= GetVehiclePedIsIn(CORRUPT.getPlayerPed(), true) then
