local a = {
    ...
    rows = {
        {"Tom", "~b~BLUE TEAM", "", false, 5, 2, 0, "1.4"},
        {"Bandit", "~b~BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"Danny", "~r~RED TEAM", "", false, 5, 0, 2, "0.5"},
        {"Tom", "~b~BLUE TEAM", "", false, 5, 0, 2, "1.4"},
        {"Bandit", "~b~BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"Danny", "~r~RED TEAM", "", false, 5, 2, 0, "0.5"},
        {"Tom", "~b~BLUE TEAM", "", false, 5, 2, 0, "1.4"},
        {"Bandit", "~b~BLUE TEAM", "", false, 7, 5, 0, "0.5"},
        {"Danny", "~r~RED TEAM", "", false, 5, 2, 0, "0.5"}
    }
...
RegisterCommand(
    ...
    function()
        if CORRUPT.isDeveloper() then
            local b = CORRUPT.createLeaderboard(a)
            Wait(1000)
            CORRUPT.closeLeaderboard(b)
        end
    end,
    false
)
...
function CORRUPT.closeLeaderboard(b)
    c[b] = false
end
function CORRUPT.createLeaderboard(m)
    local n = Scaleform("SC_LEADERBOARD")
    n.RunFunction("SET_DISPLAY_TYPE", {1})
    n.RunFunction("SET_MULTIPLAYER_TITLE", {"CORRUPT RP Event"})
...
RegisterNetEvent(
    "CORRUPT:initScoreboard",
    function(t)
        d = t
    end
)
function CORRUPT.updateScoreboard(u, v, w)
    local x
    ...
    if u == -1 then
        ...
        if x then
            ...
            print("error: CORRUPT:updateScoreboard couldn't find row with values", u, v, w)
        end
    end
end
RegisterNetEvent(
    "CORRUPT:updateScoreboard",
    function(u, v, w)
        CORRUPT.updateScoreboard(u, v, w)
    end
)
function CORRUPT.setPlayerCanOpenLeaderboard(F)
    e = F
end
function CORRUPT.clearLeaderboardData()
    d = {columns = {}, rows = {}}
end
function CORRUPT.isLeaderboardSetup()
    return table.count(d.rows) > 0
end
function CORRUPT.removePlayerFromLeaderboard(u)
