    ...
    function(b, c)
        if c then
            for d, e in pairs(a.stations) do
                tvRP.addBlip(e.x, e.y, e.z, 361, 4, "Petrol Station", 0.6)
            end
        end
    end
)
...
function CORRUPT.getVehicleFuel(l)
...
local function r(l)
    ...
    elseif not k then
        setVehicleFuel(l, CORRUPT.getVehicleFuel(l))
        k = true
    end
    ...
    SendNUIMessage({showFuel = true, fuel = CORRUPT.getVehicleFuel(l)})
end
...
AddEventHandler(
    ...
    function(z, t, l)
        ...
        if z then
            TriggerServerEvent("CORRUPT:takeAmount", math.floor(i))
        end
        local F = GetEntityModel(l)
        local G = tvRP.getVehicleIdFromHash(F)
        if G then
            local H = GetFuel(l)
            TriggerServerEvent("CORRUPT:updateFuel", G, math.floor(H * 1000) / 1000)
        end
        i = 0.0
    end
)
AddEventHandler(
    ...
    function(z, t, l)
        ...
        CORRUPT.loadAnimDict("timetable@gardener@filling_can")
                    ...
                    elseif f then
                        ...
                        if j >= a.jerryCanCost then
                            if not HasPedGotWeapon(t, 883325847) then
                                ...
                                if IsControlJustReleased(0, 38) then
                                    CORRUPT.allowweapon("883325847")
                                    GiveWeaponToPed(t, 883325847, 4500, false, true)
                                    TriggerServerEvent("CORRUPT:takeAmount", a.jerryCanCost)
                                    j = 50000
                                end
                            else
                                ...
                                if V > 0 then
                                    ...
                                    if IsControlJustReleased(0, 38) then
                                        TriggerServerEvent("CORRUPT:takeAmount", math.floor(V))
