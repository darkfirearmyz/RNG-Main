...
RegisterNetEvent(
    ...
    function()
        ...
        b.onJob = true
        CORRUPT.disableCustomizationSave(b.onJob)
        b.previousCustomisation = tvRP.getCustomization()
        local f = PlayerPedId()
        SetPedComponentVariation(f, 5, 126, 0, 0)
        SetPedComponentVariation(f, 6, 0, 0, 0)
        SetPedComponentVariation(f, 11, 449, 0, 0)
        tvRP.notify("~g~Deliveroo Job started, exit the restaurant and head to the first drop off.")
        while b.onJob do
...
RegisterNetEvent(
    "CORRUPT:deliverooJobEnd",
    function(g)
        if g then
            tvRP.notify("~g~Shift complete.")
            DeleteVehicle(GetVehiclePedIsIn(CORRUPT.getPlayerPed(), false))
            DeleteVehicle(b.tempVehicle)
        else
            RemoveBlip(b.tempBlip)
            tvRP.removeMarker(b.tempMarker)
        end
        tvRP.setCustomization(b.previousCustomisation)
        b.onJob = false
        CORRUPT.disableCustomizationSave(b.onJob)
        b.tempMarker = 0
        b.tempBlip = 0
...
RegisterNetEvent(
    "CORRUPT:deliverooJobReachedNextStop",
    function(h)
...
RegisterNetEvent(
    "CORRUPT:deliverooJobSetNextBlip",
    function(j)
        b.tempBlip = AddBlipForCoord(j.x, j.y, j.z)
        SetBlipSprite(b.tempBlip, 1)
        SetBlipRoute(b.tempBlip, true)
        b.tempMarker = tvRP.addMarker(j.x, j.y, j.z - 1, 2.0, 2.0, 1.0, 200, 20, 0, 50, 50)
        ...
        if m then
            ...
            local q = function(o)
                if IsControlJustReleased(1, 38) and not b.onJob then
                    TriggerServerEvent("CORRUPT:attemptBeginDeliverooJob")
                end
            end
            tvRP.addBlip(b.startVector.x, b.startVector.y, b.startVector.z, 106, 1, "Deliveroo Job")
            tvRP.addMarker(
                b.startVector.x,
                b.startVector.y,
                b.startVector.z - 1,
            ...
            CORRUPT.createArea("deliveroo", b.startVector, 1.5, 6, n, p, q, {})
        end
    end
)
