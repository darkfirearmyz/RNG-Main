...
RegisterNetEvent(
    ...
    function()
        ...
        a.onJob = true
        tvRP.notify("~g~Bus Job started, exit the bus station and head to the first bus stop.")
        while a.onJob do
...
RegisterNetEvent(
    ...
    function(d)
        ...
        if a.tempMarker then
            tvRP.removeMarker(a.tempMarker)
        end
        ...
        local f = CORRUPT.loadModel(b[a.stopNumber].pedModel)
        local g = CORRUPT.getPosition()
        local h = 10.0
...
RegisterNetEvent(
    "CORRUPT:setNextBusJobBlip",
    function(n)
        a.tempBlip = AddBlipForCoord(n.x, n.y, n.z)
        SetBlipSprite(a.tempBlip, 1)
        SetBlipRoute(a.tempBlip, true)
        a.tempMarker = tvRP.addMarker(n.x, n.y, n.z, 1.2, 1.2, 1.2, 255, 0, 0, 125, 50, 0, true, true)
    end
)
RegisterNetEvent("CORRUPT:onClientSpawn")
AddEventHandler(
        ...
        if p then
            ...
            local s = function(r)
                if IsControlJustReleased(1, 38) and not a.onJob then
                    TriggerServerEvent("CORRUPT:attemptBeginBusJob")
                end
            end
            tvRP.addBlip(a.startVector.x, a.startVector.y, a.startVector.z, 106, 1, "Bus Driver Job")
            tvRP.addMarker(
                a.startVector.x,
                a.startVector.y,
                a.startVector.z - 1,
            ...
            CORRUPT.createArea("bus", a.startVector, 1.5, 6, q, m, s, {})
        end
    end
)
function alert(t)
