...
RMenu:Get("scubadiving", "main"):SetSubtitle("CORRUPT | ~b~Scuba Diving")
local function j()
    RageUI.CloseAll()
...
AddEventHandler(
    ...
    function(l, m)
        if m then
            ...
            tvRP.addBlip(a.jobPosition.x, a.jobPosition.y, a.jobPosition.z, 308, 0, "Scuba Diving")
            tvRP.addMarker(
                a.jobPosition.x,
                a.jobPosition.y,
                a.jobPosition.z - 0.2,
            ...
            CORRUPT.createArea("scubajob", a.jobPosition, 1.5, 6, q, s, t, {})
        end
    end
)
RageUI.CreateWhile(
    1.0,
    RMenu:Get("scubadiving", "main"),
    nil,
    function()
        RageUI.IsVisible(
            RMenu:Get("scubadiving", "main"),
            ...
            function()
                if not e then
                    RageUI.ButtonWithStyle(
                        ...
                        function(u, v, w)
                            if w then
                                TriggerServerEvent("CORRUPT:requestScubaJob")
                            end
                        end
                    )
                end
                if d ~= 0 then
                    RageUI.ButtonWithStyle(
                        ...
                        function(u, v, w)
                            if w then
                                TriggerServerEvent("CORRUPT:claimScubaReward", c)
                                d = 0
                                c = nil
                                e = false
                                i = false
                                ...
                                tvRP.setCustomization(h)
                                local x = PlayerPedId()
                                SetEnableScuba(x, false)
                                SetPedMaxTimeUnderwater(x, 10.0)
                                CORRUPT.setMaxUnderWaterUITimenewTime(10.0)
                            end
                        end
                    )
                end
...
local function y()
    for z, A in pairs(b) do
        tvRP.removeMarker(A.objectMarker)
        tvRP.removeArea(A.objectArea)
...
function startScubaDiving()
    CORRUPT.loadAnimDict("missheistchem2")
    local x = PlayerPedId()
    ...
    CORRUPT.drawNativeNotification("Press ~INPUT_PICKUP~ to start Scuba Diving.")
    while not F do
    ...
    TriggerServerEvent("CORRUPT:finishScubaJob", c, d)
end
RegisterNetEvent(
    "CORRUPT:createScubaJob",
    function(J, K)
        c = K
        e = true
        CORRUPT.disableCustomizationSave(e)
        DoScreenFadeOut(500)
        Wait(600)
        ...
        SetBlipAlpha(L, 180)
        local q = function(r)
            CORRUPT.drawNativeNotification("Press ~INPUT_DETONATE~ to begin diving!")
        end
        local s = function(r)
        end
        local t = function(r)
            if IsControlJustReleased(1, 47) then
                ...
                local M = CORRUPT.getPlayerVehicle()
                if M ~= 0 then
                    if GetEntityModel(M) ~= `dinghy` then
                        tvRP.notify("You must be in a dinghy to start scuba diving!")
                    else
                        tvRP.removeArea("scubajob_" .. J.name)
                        f = "~y~Search for treasures and return to your boat when finished"
                        createRewardObjects(J)
                        CreateThread(startScubaDiving)
                    end
                else
                    tvRP.notify("You must be in a boat to start scuba diving!")
                end
            end
        end
        SetWaypointOff()
        local N = tvRP.addBlip(J.position.x, J.position.y, J.position.z, J.blipId, J.blipColour, "Scuba Diving")
        SetNewWaypoint(J.position.x, J.position.y)
        CORRUPT.createArea("scubajob_" .. J.name, J.position, 250, 15, q, s, t, {})
        h = tvRP.getCustomization()
        local p = math.random(1, 15)
        local O = PlayerPedId()
        ...
        TriggerServerEvent("CORRUPT:scubaSetVehicle", c, R)
        DoScreenFadeIn(2000)
        SetPedScubaGearVariation(PlayerPedId())
            ...
            function()
                while e do
                    if i then
                        ...
                        SetWaypointOff()
                        TriggerServerEvent("CORRUPT:finishScubaJob", c, 0)
                        TriggerServerEvent("CORRUPT:claimScubaReward", c)
                        d = 0
                        c = nil
                        e = false
...
function createRewardObjects(J)
    for z, A in pairs(J.rewardObjects) do
        local T = CORRUPT.loadModel(A.objectModel)
        ...
        local q = function(r)
            CORRUPT.drawNativeNotification("Press ~INPUT_PICKUP~ to collect!")
        end
        local s = function(r)
        end
        local t = function(r)
            if IsControlJustReleased(1, 38) and e then
                tvRP.notify("~g~Item collected.")
                d = d + 1
                tvRP.removeMarker(b[r.objectId].objectMarker)
                tvRP.removeArea(b[r.objectId].objectArea)
        ...
        CORRUPT.createArea("scubajob_" .. J.name .. "_obj_" .. z, A.objectPosition, 3.0, 3.0, q, s, t, {objectId = z})
        b[z] = {}
        b[z].objectId = U
        b[z].objectMarker = V
