            ...
            ontick_fishing = function(y)
                if globalFishingOnDuty then
                    if
                     ...
                     then
                        TriggerServerEvent("CORRUPT:fishingJobChecks", y.type)
                    end
...
function func_fishingStatus(y)
    ...
    elseif g and c then
        if IsControlJustPressed(0, 102) then
            ...
            if d >= e and d <= f then
                TriggerServerEvent("CORRUPT:fishingCaughtFish", d)
            else
                TriggerServerEvent("CORRUPT:fishingCaughtFish", d)
...
CORRUPT.createThreadOnTick(func_fishingStatus)
RegisterNetEvent(
    "CORRUPT:fishingJobSetCasted",
    function()
        g = true
...
CORRUPT.createThreadOnTick(func_createBar)
