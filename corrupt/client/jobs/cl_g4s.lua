...
local function B()
    ...
    local D = 500 * math.ceil(GetDistanceBetweenCoords(CORRUPT.getPlayerCoords(), GetEntityCoords(b), true) / 10)
    if j then
...
tvRP.addMarker(d.x, d.y, d.z, 1.2, 1.2, 1.2, 0, 255, 125, 125, 50, 0, true, true)
local F = tvRP.addBlip(d.x, d.y, d.z, 457, 18, "G4S Depot", 1.0, false)
local G = function()
...
CORRUPT.createArea("g4sDepot_", d, 1.5, 6, G, H, I)
    ...
    function()
        while true do
            if not y.hackingEngaged and not a and not y.awaitingResponse then
                if table.count(s) > 0 and CORRUPT.getPlayerVehicle() == 0 then
                    for K, L in pairs(s) do
                        ...
                        if table.has(p, O) or not DoesEntityExist(N) then
                            ...
                            if IsVehicleStopped(N) then
                                local P = #(CORRUPT.getPlayerCoords() - GetEntityCoords(N))
                                ...
                                if P <= 11.0 then
                                    ...
                                    if IsControlJustPressed(1, 18) then
                                        ...
                                        y.entity = N
                                        y.src = O
                                        TriggerServerEvent("CORRUPT:requestHackingChanceServer", M, O)
...
function toggleCashbox()
    if A.active then
        ...
        local o = CORRUPT.loadModel("cashbox")
        local C = CORRUPT.getPlayerPed()
...
RegisterNetEvent(
    ...
    function(b, W, X, Y)
        if Y then
            ...
            local o = CORRUPT.loadModel("prop_cs_tablet")
            v = CreateObject(o, 0, 0, 0, true, true, true)
            AttachEntityToEntity(
                v,
                CORRUPT.getPlayerPed(),
                GetPedBoneIndex(CORRUPT.getPlayerPed(), 57005),
                0.17,
                0.10,
                -0.13,
                24.0,
            ...
            if HasScaleformMovieLoaded(R) then
                ...
                if IsScaleformMovieMethodReturnValueReady(S) then
                    ...
                    elseif ProgramID == 87 then
                        ...
                        TriggerServerEvent("CORRUPT:updateHackingOutcome", y.netId, y.src, U, false)
                        DeleteObject(v)
                        break
                    ...
                    elseif ProgramID == 86 then
                        ...
                        TriggerServerEvent("CORRUPT:updateHackingOutcome", y.netId, y.src, U, true)
                        DeleteObject(v)
                        break
...
RageUI.CreateWhile(
    ...
    function()
        RageUI.IsVisible(
            ...
            function()
                if not j then
                    if CORRUPT.getPlayerVehicle() == 0 and w < 4 then
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(Z, _, a0)
                            if a0 then
                                TriggerServerEvent("CORRUPT:toggleShiftG4S", false)
                                a = false
                            end
                        end
                    )
                    ...
                    if not h then
                        RageUI.ButtonWithStyle(
                            ...
                            function(Z, _, a0)
                                if a0 then
                                    TriggerServerEvent("CORRUPT:activatePanicServer", c)
                                    imageNotification(
                                        "notification_images",
                                        "g4slogo",
                ...
                if not a then
                    RageUI.ButtonWithStyle(
                        ...
                        function(Z, _, a0)
                            if a0 then
                                RageUI.Visible(RMenu:Get("g4s", "shiftHandler"), false)
                                TriggerServerEvent("CORRUPT:toggleShiftG4S", true)
                            end
                        end
                    )
                else
...
function newG4sVehicle()
    ...
    if DoesEntityExist(b) and not IsVehicleDriveable(b, true) then
        ...
        b = CORRUPT.spawnVehicle(n, a1.x, a1.y, a1.z, GetEntityHeading(C), true, true)
        c = VehToNet(b)
        TriggerServerEvent("CORRUPT:updateCurrentVehicle", c)
        w = w + 1
    end
end
function securityCheck(C)
    local a2 = CORRUPT.getPlayerVehicle()
    if a2 ~= 0 and a2 ~= nil and b ~= nil and b ~= 0 then
        if a2 ~= b then
            ...
            SetTimeout(
                15000,
                function()
                    a2 = GetVehiclePedIsIn(CORRUPT.getPlayerPed(), false)
                    if a2 ~= 0 then
                        if a2 ~= b then
                            TriggerServerEvent("CORRUPT:toggleShiftG4S", false)
                            tvRP.removeBlip(k)
...
RegisterNetEvent(
    "CORRUPT:resetPanicStatus",
    function()
        h = false
    end
)
RegisterNetEvent(
    "CORRUPT:updateJobInformation",
    function(a3)
        if l ~= 0 then
            tvRP.removeBlip(l)
        end
        if a3.jobActive then
            ...
            Wait(3000)
            TriggerServerEvent("CORRUPT:acceptJob")
        end
    end
)
...
RegisterNetEvent(
    "CORRUPT:startShiftG4S",
    function(a4)
        local C = CORRUPT.getPlayerPed()
        local J = CORRUPT.getPlayerCoords()
        a = true
        CORRUPT.disableCustomizationSave(a)
        b = CORRUPT.spawnVehicle(n, z.x, z.y, z.z, z.heading, true, true)
        c = VehToNet(b)
        ...
        x = json.encode(tvRP.getCustomization())
        if a8 then
            CORRUPT.loadCustomisationPreset("G4SMale")
        else
            CORRUPT.loadCustomisationPreset("G4SFemale")
        end
        Wait(2000)
        ...
        Wait(5000)
        TaskVehicleTempAction(CORRUPT.getPlayerPed(), b, 27, 10.0)
        Wait(2000)
...
RegisterNetEvent(
    ...
    function()
        ...
        local C = CORRUPT.getPlayerPed()
        tvRP.setCustomization(json.decode(x))
        if A.active then
            toggleCashbox()
        end
        if k ~= 0 then
...
RegisterNetEvent(
    ...
    function()
        ...
        while not f do
            ...
            local P = #(CORRUPT.getPlayerCoords() - ae)
            if P <= 1.0 then
                ad = true
        ...
        TriggerServerEvent("CORRUPT:updateMoneyInVehicle")
        t = false
    end
)
RegisterNetEvent(
    "CORRUPT:requestMoneyOutVehicle",
    function()
        local ac = CORRUPT.getPlayerPed()
        while IsPedInAnyVehicle(ac, true) do
            Wait(400)
        ...
        w = 1
        TriggerServerEvent("CORRUPT:updateMoneyOutVehicle")
    end
)
...
RegisterNetEvent(
    "CORRUPT:activeVansUpdate",
    function(ax, O, aC)
        if not a then
            local b = CORRUPT.getObjectId(ax, "CORRUPT:activeVansUpdate 1")
            if b ~= 0 and b ~= nil and DoesEntityExist(b) then
                if aC then
                    s[ax] = nil
                else
                    if not table.has(s, ax) then
                        local b = CORRUPT.getObjectId(ax, "CORRUPT:activeVansUpdate")
                        s[ax] = {O, b, ax}
                    end
                end
            end
