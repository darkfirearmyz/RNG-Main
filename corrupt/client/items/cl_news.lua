...
function createNewsItemInitally(i)
    ...
    CORRUPT.loadAnimDict(l.animations.dict)
...
function removeNewsItem(i)
    if a.items[i].holding then
        ...
        DetachEntity(CORRUPT.getObjectId(a.items[i].netId, "removeNewsItem (1)"), true, true)
        DeleteEntity(CORRUPT.getObjectId(a.items[i].netId, "removeNewsItem (2)"))
        a.items[i].netId = nil
            ...
            for i, s in pairs(a.items) do
                if a.items[i].holding then
                    if not IsEntityPlayingAnim(j, a.items[i].animations.dict, a.items[i].animations.name, 3) then
                        CORRUPT.loadAnimDict(a.items[i].animations.dict)
                        TaskPlayAnim(j, 1.0, -1, -1, 50, 0, 0, 0, 0, false, false)
                        TaskPlayAnim(
                            j,
