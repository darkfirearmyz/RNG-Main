...
local l = 0
WeaponNames = {}
local m = module("corrupt-assets", "cfg/weapons")
...
local w = -1
RegisterNetEvent(
    "CORRUPT:setNHSCallerId",
    function(x)
        w = x
    end
)
AddEventHandler(
    "CORRUPT:countdownEnded",
    function()
        e = true
    end
)
Citizen.CreateThread(
    function()
        while true do
            if IsDisabledControlJustPressed(0, 38) and not CORRUPT.playerInWager() then
                if b and not g then
                    local y = GetEntityCoords(GetPlayerPed(-1))
                    if not CORRUPT.isPlayerInRedZone() and not CORRUPT.isPlayerInTurf() then
                        PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
                        TriggerServerEvent("CORRUPT:NHSComaCall")
                        TriggerEvent("CORRUPT:DEATH_SCREEN_NHS_CALLED")
                    end
                    g = true
                elseif e and b then
                    PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
                    TriggerEvent("CORRUPT:CLOSE_DEATH_SCREEN")
                    CORRUPT.respawnPlayer()
                    g = false
                    if w ~= -1 then
                        TriggerServerEvent("CORRUPT:endNHSCall", w)
                    end
                    TriggerEvent("CORRUPT:respawnKeyPressed")
                    TriggerServerEvent("CORRUPT:SendSpawnMenu")
                end
                Wait(1000)
            end
            Wait(0)
...
local function z()
    local A = CORRUPT.getPlayerCoords()
        ...
        while true do
            ...
            if IsEntityDead(GetPlayerPed(-1)) and not b then
                ...
                if currentBackpack then
                    TriggerEvent("CORRUPT:removeBackpack")
                end
                if not CORRUPT.globalOnPoliceDuty() then
                    TriggerServerEvent("CORRUPT:forceStoreWeapons")
                end
                tvRP.ejectVehicle()
                b = true
                h = F
                TriggerServerEvent("CORRUPT:getNumOfNHSOnline")
                CORRUPT.setArmour(0)
                CORRUPT.updateHealth(true)
                Wait(250)
            end
            if f <= 0 then
                f = 100
                ...
                CORRUPT.setHealth(102)
                SetEntityInvincible(GetPlayerPed(-1), true)
                a = getRandomComaAnimation()
                CORRUPT.loadAnimDict(a.dict)
        ...
        while true do
            if b then
                ...
                if not IsEntityDead(I) then
                    ...
                    if not IsEntityPlayingAnim(I, a.dict, a.anim, 3) and not CORRUPT.isUsingStretcher() then
                        if a.dict ~= nil then
                            CORRUPT.loadAnimDict(a.dict)
...
RegisterNetEvent("Corrupt:Revive", function()
    local C = GetPlayerPed(-1)
    ...
    TriggerServerEvent("CORRUPT:playerRespawned")
end)
RegisterNetEvent(
    "CORRUPT:getNumberOfDocsOnline",
    function(L)
        if not CORRUPT.isPurge() then
            if CORRUPT.isPlayerInRedZone() or CORRUPT.isPlayerInTurf() then
        ...
        if not CORRUPT.isPurge() then
            if GetVehiclePedIsIn(PlayerPedId(), false) ~= 0 then
                ...
                TriggerEvent("CORRUPT:IsInMoneyComa", true)
                ExecuteCommand("storeallweapons")
                if not CORRUPT.globalOnPoliceDuty() then
                    TriggerServerEvent("CORRUPT:InComa")
                end
                TriggerServerEvent("CORRUPT:MoneyDrops")
            end
        end
        CreateThread(
            function()
                local N = GetGameTimer()
                while CORRUPT.getKillerInfo().ready == nil do
                    Wait(0)
                end
                local O = CORRUPT.getKillerInfo()
                local P = false
                if O.name == nil then
                    P = true
                end
                e = false
                if not CORRUPT.isUserHidden(O.user_id) then
                    TriggerEvent("CORRUPT:SHOW_DEATH_SCREEN", d, O.name or "N/A", O.user_id or "N/A", O.weapon or "N/A", P)
                else
                    TriggerEvent("CORRUPT:SHOW_DEATH_SCREEN", d, "N/A", "N/A", "N/A", P)
                end
            end
        )
        if not CORRUPT.isPurge() then
...
RegisterNetEvent(
    ...
    function(R)
        ...
        TriggerEvent("CORRUPT:openSpawnUI", T)
    end
)
AddEventHandler(
    "CORRUPT:respawnButtonClicked",
    function(U, V)
        if V and V > 0 then
            TriggerServerEvent("CORRUPT:takeAmount", V)
        end
...
function CORRUPT.respawnPlayer()
...
local Y = false
function CORRUPT.isTazed(Z)
    return Y or IsPedBeingStunned(PlayerPedId(), 0) or CORRUPT.isPedBeingTackled() or
        isInGreenzone and not globalOnPoliceDuty and not Z
end
function CORRUPT.isTazedByRevive()
    return Y
end
RegisterNetEvent(
    "TriggerTazer",
...
local O = {}
function CORRUPT.getKillerInfo()
    return O
end
Citizen.CreateThread(
    function()
        ...
        while true do
            Citizen.Wait(0)
            if IsEntityDead(PlayerPedId()) and not CORRUPT.isPedScriptGuidChanging() then
                Citizen.Wait(500)
                ...
                local a8 = CORRUPT.getPedServerId(a7)
                if inOrganHeist then
                    TriggerServerEvent("CORRUPT:diedInOrganHeist", a8)
                    CORRUPT.setDeathInOrganHeist()
                end
                local a9 = false
                local aa = 0
                if a4 == PlayerId() or a4 == nil then
                    ...
                    local ab = CORRUPT.getPlayerName(a4)
                    O.name = ab
                    aa = #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(a7))
                end
                O.source = a8
                O.user_id = CORRUPT.getUserId(a8)
                O.weapon = tostring(a6)
                O.ready = true
                local _q, _r = GetPedLastDamageBone(CORRUPT.getPlayerPed())
                if GetGameTimer() < i + 10000 then
                    local a8 = CORRUPT.getPedServerId(a7)
                    TriggerServerEvent("CORRUPT:onPlayerKilled", "killed", a8, a6, a9, aa, (_r == 31086))
                    l = 0
                elseif GetGameTimer() < i + j then
                    local a8 = CORRUPT.getPedServerId(a7)
                    j = 0
                    TriggerServerEvent("CORRUPT:onPlayerKilled", "finished off", a8, a6)
                    l = 0
                end
                a4 = nil
                a3 = nil
...
function tvRP.varyHealth(ac)
    local C = PlayerPedId()
    local ad = math.floor(GetEntityHealth(C) + ac)
    CORRUPT.setHealth(ad)
end
function CORRUPT.setFriendlyFire(ae)
