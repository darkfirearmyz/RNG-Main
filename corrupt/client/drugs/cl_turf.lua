...
Citizen.CreateThread(
    function()
        Wait(5000)
        local j = tvRP.addBlip(-552.90362548828, 5326.1733398438, 72.599586486816, 403, 3, "Meth Trader")
        local k = tvRP.addBlip(133.73462677002, -1305.0340576172, 29.268741607666, 403, 4, "Cocaine Trader")
        local l = tvRP.addBlip(103.47916412354, -1937.1091308594, 19.803705215454, 140, 2, "Weed Trader")
        local m = tvRP.addBlip(3569.8334960938, 3662.5893554688, 33.920593261718, 586, 1, "Heroin Trader")
        local n = tvRP.addBlip(-1118.4926757813, 4926.1889648438, 218.35691833496, 150, 1, "Large Arms Trader")
	    local o = tvRP.addBlip(5065.6201171875, -4591.3857421875, 2.8652405738831, 150, 1, "Large Arms Trader")
        local p = tvRP.addBlip(1317.0300292969, 4309.8359375, 38.005485534668, 604, 8, "LSD North Trader", 1.0)
        local q = tvRP.addBlip(2485.7429199219, -430.13989257813, 92.992835998535, 604, 8, "LSD South Trader", 1.0)
        tvRP.addBlip(-349.78021240234, -2708.0966796875, 6.0439453125, 150, 1, "Mosin City") 
        tvRP.addBlip(946.41760253906, -1697.7493896484, 30.07177734375, 227, 27, "Corrupt Drift Club")
        local r = {
            {
                title = "Weed Trader",
                type = "radius",
...
AddEventHandler(
    ...
    function(v, w)
        if w then
            TriggerServerEvent("CORRUPT:refreshTurfOwnershipData")
            TriggerServerEvent("CORRUPT:requestDrugPriceUpdate")
        end
    end
)
RegisterNetEvent(
    "CORRUPT:attackGangCapture",
    function(x, y)
        f = true
        a = y
...
RegisterNetEvent(
    "CORRUPT:updateTurfOwner",
    function(x, z)
        turfData[x].gangOwner = z
    end
)
RegisterNetEvent(
    "CORRUPT:defendGangCapture",
    function(x, y)
        a = y
        e = true
...
RegisterNetEvent(
    "CORRUPT:turfDefenseInterrupt",
    function()
        e = false
    end
)
RegisterNetEvent(
    "CORRUPT:initiateGangCaptureCheck",
    function(y, B)
        b[y] = B
    end
)
RegisterNetEvent(
    "CORRUPT:gotTurfOwnershipData",
    function(C)
...
RegisterNetEvent(
    "CORRUPT:setTurfDataProfit",
    function(x, D)
        turfData[x].profit = math.floor(D)
    end
)
function CORRUPT.isPlayerInTurf()
    for D, E in pairs(h) do
        if #(CORRUPT.getPlayerCoords() - E.position) < E.distance then
            return true
        end
    end
    return false
    ...
    function(v, w)
        if w then
            ...
            local O = function(M)
                if IsControlJustPressed(1, 51) then
                    if PlayerIsInGang then
                        tvRP.notify("~r~You need to be in a gang to capture a turf!")
                    elseif not f then
                        TriggerServerEvent("CORRUPT:refreshTurfOwnershipData")
                        TriggerServerEvent("CORRUPT:checkTurfCapture", M.turfId)
                    end
                    Wait(10000)
                end
            end
            for F, P in pairs(h) do
                CORRUPT.createArea("turf_" .. F, P.position, 1.5, 6, L, N, O, {turfId = F})
            end
            local Q = function(M)
                            ...
                            if T ~= nil and tonumber(T) then
                                ...
                                if T <= 25 and T >= 0 then
                                    ...
                                    if GetGameTimer() > K + 30000 then
                                        ...
                                        if F == 1 then
                                            TriggerServerEvent("CORRUPT:setNewWeedPrice", T)
                                        elseif F == 2 then
                                            TriggerServerEvent("CORRUPT:setNewCocainePrice", T)
                                        elseif F == 3 then
                                            TriggerServerEvent("CORRUPT:setNewMethPrice", T)
                                        elseif F == 4 then
                                            TriggerServerEvent("CORRUPT:setNewHeroinPrice", T)
                                        elseif F == 5 then
                                            TriggerServerEvent("CORRUPT:setNewLargeArmsCommission", T)
                                        elseif F == 6 then
                                            TriggerServerEvent("CORRUPT:setNewLSDNorthPrice", T)
                                        elseif F == 7 then
                                            TriggerServerEvent("CORRUPT:setNewLSDSouthPrice", T)
                                        end
                                    else
                                        tvRP.notify("~r~Rate limit, please wait 30s to change the commission.")
                                    end
                                end
                            end
                        end
            ...
            for F, G in pairs(i) do
                CORRUPT.createArea("turfmanager_" .. F, G.position, 1.5, 6, Q, R, S, {turfId = F, draw = false})
            end
        end
    end
)
RegisterNetEvent(
    "CORRUPT:captureOwnershipReturned",
    function(F, U, V)
        if not U then
            startGangCapture(F, h[F].position, V)
        else
            tvRP.notify("~r~Turf already owned!")
        end
    end
)
function otherMembersStartGangCapture(x, y, W)
    if b[y] then
        ...
        Citizen.CreateThread(
            function()
                local tmpMarker = tvRP.addMarker(W.x, W.y, W.z - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
                while f do
                    local X = GetEntityCoords(CORRUPT.getPlayerPed(), false)
                    local Y = #(X - W)
                    if Y >= 37.5 then
                        tvRP.notify("~r~Turf capture cancelled, you got too far!")
                        f = false
                        TriggerServerEvent("CORRUPT:gangDefenseLocationUpdate", y, "Attackers", true)
                    elseif GetEntityHealth(CORRUPT.getPlayerPed()) <= 102 then
                        tvRP.notify("~r~Turf capture failed, you died")
                        f = false
                        TriggerServerEvent("CORRUPT:gangDefenseLocationUpdate", y, "Attackers", true)
                    end
                    Wait(1000)
                end
                tvRP.removeMarker(tmpMarker)
            end
        )
        Citizen.CreateThread(
            function()
...
function startGangCapture(x, W, y)
    if turfData[x].owned then
        tvRP.notify("~r~You already own this turf!")
        TriggerServerEvent("CORRUPT:failCaptureTurfOwned", x)
    else
        TriggerServerEvent("CORRUPT:initiateGangCapture", x, y)
        local timer = g
        while b[y] == nil do
            Wait(0)
        end
        if b[y] then
            ...
            Citizen.CreateThread(
                function()
                    tmpMarker = tvRP.addMarker(W.x, W.y, W.z - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
                    while f do
                        local X = GetEntityCoords(CORRUPT.getPlayerPed(), false)
                        local Y = #(X - W)
                        if Y >= 37.5 then
...
function defendGangCapture(x, a, Z, _, a0)
    ...
    Citizen.CreateThread(
        function()
            local tmpMarker = tvRP.addMarker(Z, _, a0 - 10, 75, 75, 75, 0, 200, 0, 50, 1000)
            while e do
                local X = GetEntityCoords(CORRUPT.getPlayerPed(), false)
                local Y = #(X - vector3(Z, _, a0))
                if Y < 37.5 and GetEntityHealth(CORRUPT.getPlayerPed()) > 102 then
                    TriggerServerEvent("CORRUPT:gangDefenseLocationUpdate", a, "Defenders", true)
                elseif GetEntityHealth(CORRUPT.getPlayerPed()) > 102 then
                    TriggerServerEvent("CORRUPT:gangDefenseLocationUpdate", a, "Defenders", false)
                elseif GetEntityHealth(CORRUPT.getPlayerPed()) <= 102 then
                    tvRP.notify("~r~Turf defense failed, you died")
                    e = false
                    TriggerServerEvent("CORRUPT:gangDefenseLocationUpdate", a, "Defenders", false)
                else
                    TriggerServerEvent("CORRUPT:gangDefenseLocationUpdate", a, "Defenders", false)
...
function gangCaptureSuccess(x, a)
    ...
    TriggerServerEvent("CORRUPT:gangCaptureSuccess", a)
end
function gangDefenseSuccess()
    TriggerServerEvent("CORRUPT:gangDefenseSuccess", a)
end
