...
local b = {
    ...
    ["Heroin"] = {
        ...
        ["processing"] = {position = vector3(1593.9248046875, 3585.1362304688, 38.964069366456), radius = 100}
    },
    ["LSD"] = {
...
RegisterNetEvent(
    "CORRUPT:playGrindingScenario",
    function(c, d)
        if not a then
            a = true
            local e = GetGameTimer()
            TaskStartScenarioInPlace(CORRUPT.getPlayerPed(), c, 0, true)
            local f
            if d then
...
RegisterNetEvent(
    ...
    function()
        if not a then
            ...
            local j = CORRUPT.loadModel("prop_tool_pickaxe")
            ...
            tvRP.notify("Mining currently in progress.")
        end
    end
)
local l = false
            ...
            local u = function(s)
                if s.nearby then
                    if IsControlJustReleased(0, 38) and GetVehiclePedIsIn(PlayerPedId(), false) == 0 then
                        if not a then
                            m()
                            TriggerServerEvent("CORRUPT:requestGrinding", s.drug, s.type)
                            Wait(500)
                        else
                            tvRP.notify("Action in progress, please wait.")
                        end
                    end
                end
            end
...
Citizen.CreateThread(
    function()
        ...
        local A = CORRUPT.loadModel("a_c_hen")
