...
RegisterNetEvent(
    ...
    function()
        ...
        local q = CORRUPT.loadModel("a_c_hen")
        RequestAnimDict(o)
        while not HasAnimDictLoaded(o) do
            Wait(0)
        end
        TaskPlayAnim(CORRUPT.getPlayerPed(), o, p, 2.0, 2.0, -1, 51, 0, false, false, false)
        RemoveAnimDict(o)
    ...
    while GetGameTimer() - z < w do
        ...
        if D > 1.0 then
            ...
            if y == 5 then
                y = 0
                local E = B - C
                local F = GetEntityVelocity(CORRUPT.getPlayerPed())
                SetEntityCoordsNoOffset(CORRUPT.getPlayerPed(), C.x + E.x, C.y + E.y, C.z)
                ForcePedMotionState(CORRUPT.getPlayerPed(), -1115154469, 1, 1, 0)
                SetEntityVelocity(CORRUPT.getPlayerPed(), F.x, F.y, F.z)
            end
            B = GetEntityCoords(CORRUPT.getPlayerPed())
        end
        DrawToons()
        DrawCubes()
        if e and not f then
...
InitPed = function()
    ...
    while GetScreenCoordFromWorldCoord(K.x, K.y, K.z) and not IsPointOnRoad(K.x, K.y, K.z) do
        I = math.random(0, 359)
        J = math.random(50, 80)
        K = GetEntityCoords(CORRUPT.getPlayerPed()) + PointOnSphere(0.0, I)
        Citizen.Wait(0)
    end
...
TrackEnt = function()
    while true do
        local D = GetVecDist(GetEntityCoords(CORRUPT.getPlayerPed()), GetEntityCoords(g))
        if D > 5.0 then
            TaskGoToEntity(g, CORRUPT.getPlayerPed(), -1, 4.0, 100.0, 1073741824, 0)
            Wait(1000)
        else
            ...
            if not d or GetGameTimer() - d > 1000 then
                d = GetGameTimer()
                TaskTurnPedToFaceEntity(g, CORRUPT.getPlayerPed(), -1)
            end
...
DrawCubes = function()
    local W = GetEntityCoords(CORRUPT.getPlayerPed())
    local S = 10
    for T, U in pairs(b) do
        local X = 0.1
        local Y = 0.1
...
RegisterNetEvent(
    ...
    function()
        if ad then
            ...
            while s < 10 do
                tvRP.varyHealth(-10)
                tvRP.notify("I don't feel too good..")
                Wait(5000)
                s = s + 1
            end
