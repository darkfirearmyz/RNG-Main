...
function createSitAtBarScene(o, p)
    ...
    NetworkAddPedToSynchronisedScene(CORRUPT.getPlayerPed(), q, a, animName, 2.0, -1000, 9, 0, 1148846080, 0)
    ...
    function()
        while true do
            if insideDiamondCasino then
                if e then
                    SetPedCapsule(CORRUPT.getPlayerPed(), 0.13)
                    if IsControlJustPressed(0, 202) then
                        CORRUPT.loadAnimDict("anim@amb@office@seating@male@var_a@base@")
                        TaskPlayAnim(
                            CORRUPT.getPlayerPed(),
