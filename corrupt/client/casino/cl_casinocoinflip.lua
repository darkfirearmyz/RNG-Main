...
Citizen.CreateThread(
    function()
        Wait(1000)
        TriggerServerEvent("CORRUPT:requestCoinflipTableData")
        for q = 0, #c.coinflipTables do
            local D = c.coinflipTables[q]
            local E = CORRUPT.loadModel(D.tableModel)
            ...
            if d then
                ...
                if
                    not l and IsControlJustPressed(0, 202) and not g and
                        not RageUI.Visible(RMenu:Get("corruptcoinflip", "confirm"))
                 then
                    local L = CORRUPT.loadAnimDict("anim_casino_b@amb@casino@games@shared@player@")
                    TaskPlayAnim(CORRUPT.getPlayerPed(), L, "sit_exit_left", 1.0, 1.0, 2500, 0, 0.0, false, false, false)
                    RemoveAnimDict("anim_casino_b@amb@casino@games@shared@player@")
                    d = false
                    e = true
                    showHowToCoinflip(false)
                    TriggerServerEvent("CORRUPT:cancelCoinflip", true)
                    TriggerServerEvent("CORRUPT:leaveCoinflipTable")
                    h = {}
                    m = false
                    SetTimeout(
                        5000,
            ...
            if not d then
                if a ~= -1 and b < 2 then
                    if IsControlJustPressed(0, 38) then
                        if f[a] == false then
                            TriggerServerEvent("CORRUPT:requestSitAtCoinflipTable", a)
                        end
                    end
                end
            end
...
function goToCoinflipSeat(o)
    ...
    TaskGoStraightToCoord(CORRUPT.getPlayerPed(), S.x, S.y, S.z, 1.0, 5000, T.z, 0.01)
    Wait(500)
    local U = v(o)
    local V = A(o)
...
function showHowToCoinflip(X)
    RageUI.CloseAll()
    RageUI.Visible(RMenu:Get("corruptcoinflip", "instructions"), X)
end
function showCoinflipMenu(X)
    RageUI.CloseAll()
    RageUI.Visible(RMenu:Get("corruptcoinflip", "mainmenu"), X)
end
RMenu.Add(
    "corruptcoinflip",
    "instructions",
...
RMenu.Add(
    ...
    RageUI.CreateSubMenu(
        RMenu:Get("corruptcoinflip", "mainmenu"),
        "",
        "~b~COINFLIP",
        CORRUPT.getRageUIMenuWidth(),
        CORRUPT.getRageUIMenuHeight(),
...
RageUI.CreateWhile(
    ...
    function()
        RageUI.IsVisible(
            RMenu:Get("corruptcoinflip", "instructions"),
            true,
            false,
            true,
            function()
                    ...
                    function(Y, Z, _)
                        if _ then
                            ...
                            if tonumber(a0) then
                                a0 = tonumber(a0)
                                if a0 > 0 then
                                    TriggerServerEvent("CORRUPT:proposeCoinflip", a0)
                                end
                            end
                        end
                    end
                        ...
                        RageUI.ButtonWithStyle(
                            ...
                            function(Y, Z, _)
                                if _ then
                                    ...
                                    if M.user_id == CORRUPT.getUserId() then
                                        k = true
                                    else
                                        k = false
                                    end
                            ...
                            RMenu:Get("corruptcoinflip", "confirm")
                        )
                    end
                else
            ...
            function()
                ...
                if k then
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(Y, Z, _)
                            if _ then
                                TriggerServerEvent("CORRUPT:cancelCoinflip")
                            end
                        end,
                        RMenu:Get("corruptcoinflip", "mainmenu")
                    )
                    ...
                    RageUI.ButtonWithStyle(
                        ...
                        function(Y, Z, _)
                            if _ then
                                TriggerServerEvent("CORRUPT:acceptCoinflip", i)
                            end
                        end,
                        RMenu:Get("corruptcoinflip", "mainmenu")
                    )
...
RegisterNetEvent(
    "CORRUPT:addCoinflipProposal",
    function(a1, M)
        h[a1] = M
    end
)
RegisterNetEvent(
    "CORRUPT:cancelCoinflipBet",
    function(a1)
        h[a1] = nil
    end
)
RegisterNetEvent(
    "CORRUPT:clearCoinflipBets",
    function(a1)
        h = {}
    end
)
RegisterNetEvent("CORRUPT:coinflipOutcome",function(a4, a5)
        l = true
        ...
        local ac = 0.00
        local F = CORRUPT.getPlayerCoords()
        while not a8 do
