...
RMenu:Get("CORRUPTChips", "cashier"):SetSubtitle("CHIPS")
RMenu.Add(
    "casino_enter",
    "casino",
    RageUI.CreateMenu(
...
RageUI.CreateWhile(
    1.0,
    RMenu:Get("CORRUPTChips", "cashier"),
    nil,
    function()
        RageUI.IsVisible(
            RMenu:Get("CORRUPTChips", "cashier"),
            ...
            function()
                    ...
                    function(l, m, n)
                        if n then
                            ...
                            if tonumber(o) then
                                o = tonumber(o)
                                if o > 0 then
                                    TriggerServerEvent("CORRUPT:buyChips", o)
                                else
                                    tvRP.notify("Negative amount.")
                                end
                            else
                ...
                RageUI.ButtonWithStyle(
                    ...
                    function(l, m, n)
                        if n then
                            TriggerServerEvent("CORRUPT:buyChips")
                        end
                    end
                )
                RageUI.ButtonWithStyle(
                    ...
                    function(l, m, n)
                        if n then
                            ...
                            if tonumber(o) then
                                o = tonumber(o)
                                if o > 0 then
                                    TriggerServerEvent("CORRUPT:sellChips", o)
                                else
                                    tvRP.notify("Negative amount.")
                                end
                            else
                ...
                RageUI.ButtonWithStyle(
                    ...
                    function(l, m, n)
                        if n then
                            TriggerServerEvent("CORRUPT:sellChips")
                        end
                    end
                )
            end,
        ...
        RageUI.IsVisible(
            ...
            function()
                RageUI.ButtonWithStyle(
                    ...
                    function(l, m, n)
                        if n then
                            TriggerServerEvent("CORRUPT:exitDiamondCasino")
                            tvRP.teleport(e.x, e.y, e.z)
                            if cardObjects then
        ...
        RageUI.IsVisible(
            ...
            function()
                RageUI.ButtonWithStyle(
                    ...
                    true,
                    function(l, m, n)
                        if n and tvRP.getPlayerCombatTimer() == 0 then
                            TriggerServerEvent("CORRUPT:enterDiamondCasino")
                            tvRP.teleport(f.x, f.y, f.z)
                        end
                    end
                )
                RageUI.ButtonWithStyle(
        ...
        RageUI.IsVisible(
            RMenu:Get("casino_rooftop_exit", "casino"),
            true,
            true,
            true,
            function()
...
function showCasinoChipsCashier(s)
    RageUI.CloseAll()
    RageUI.Visible(RMenu:Get("CORRUPTChips", "cashier"), s)
end
...
Citizen.CreateThread(
    function()
        while true do
            local t = GetEntityCoords(CORRUPT.getPlayerPed())
            a = #(t - e)
            b = #(t - f)
            c = #(t - g)
            Wait(100)
...
RegisterNetEvent("CORRUPT:setDisplayChips")
AddEventHandler(
    "CORRUPT:setDisplayChips",
    function(v)
        d = getMoneyStringFormatted(tostring(v))
    end
)
RegisterNetEvent(
    "CORRUPT:chipsUpdated",
    function()
    ...
    function(x, y)
        if y then
            ...
            for C, k in pairs(h) do
                tvRP.addBlip(k.entryPosition.x, k.entryPosition.y, k.entryPosition.z, 683, 0, "Chips Cashier", 0.7, true)
                tvRP.addMarker(
                    k.entryPosition.x,
                    k.entryPosition.y,
                    k.entryPosition.z,
                ...
                CORRUPT.createArea("chipscashier_" .. C, k.entryPosition, 1.5, 6, z, y, A, {})
            end
            for D, E in pairs(w) do
                CORRUPT.createArea("casinos_" .. D,E.position,E.radius,6,function()InCasinoZone = true end,function()InCasinoZone = false end,B,{})
            end
        end
    end
)
