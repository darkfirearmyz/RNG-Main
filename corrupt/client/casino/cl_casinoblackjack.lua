...
RegisterCommand(
    "getcasinotable",
    function()
        local K = GetEntityCoords(CORRUPT.getPlayerPed())
        ...
        for O = 0, #G.blackjackTables, 1 do
            ...
            CORRUPT.createDynamicPed(
                ...
                function(Q)
                    ...
                    CORRUPT.loadAnimDict(dealerAnimDict)
                    if dealerModel == "S_M_Y_Casino_01" then
                        TaskPlayAnim(Q, dealerAnimDict, "idle", 1000.0, -2.0, -1, 2, 1148846080, 0, false, false)
                    else
                        TaskPlayAnim(Q, dealerAnimDict, "female_idle", 1000.0, -2.0, -1, 2, 1148846080, 0, false, false)
                    end
...
function resetDealerIdle(U)
    ...
    if DoesEntityExist(U) then
        ...
        TaskPlayAnim(U, dealerAnimDict, genderAnimString .. "idle", 1000.0, -2.0, -1, 2, 1148846080, 0, false, false)
                ...
                if IsControlJustPressed(0, 201) then
                    if tonumber(z) >= 0 then
                        ...
                        elseif isChairHighRollers(F) and z < B then
                            tvRP.notify("~r~Minimum bet is " .. tostring(getMoneyStringFormatted(B)))
                        elseif z > 1000000000 then
                            tvRP.notify("~r~Maximum bet is £1,000,000,000")
                        else
...
RMenu:Get("corruptblackjack", "instructions"):SetSubtitle("~b~BLACKJACK")
RMenu.Add(
    "corruptblackjack_high",
    "instructions",
    RageUI.CreateMenu(
        "",
        "",
...
RMenu:Get("corruptblackjack_high", "instructions"):SetSubtitle("~b~BLACKJACK")
RageUI.CreateWhile(
    1.0,
    RMenu:Get("corruptblackjack", "instructions"),
    nil,
    function()
        RageUI.IsVisible(
            RMenu:Get("corruptblackjack", "instructions"),
        ...
        RageUI.IsVisible(
            RMenu:Get("corruptblackjack_high", "instructions"),
            true,
            true,
            true,
            function()
...
function showHowToBlackjack(a3)
    ...
    if not isChairHighRollers(a) then
        RageUI.Visible(RMenu:Get("corruptblackjack", "instructions"), a3)
    else
        RageUI.Visible(RMenu:Get("corruptblackjack_high", "instructions"), a3)
    end
end
...
function goToBlackjackSeat(an)
    ...
    c = an
    fVar3 = blackjack_func_217(CORRUPT.getPlayerPed(), blackjack_func_218(c, 0), 1)
    fVar4 = blackjack_func_217(CORRUPT.getPlayerPed(), blackjack_func_218(c, 1), 1)
    fVar5 = blackjack_func_217(CORRUPT.getPlayerPed(), blackjack_func_218(c, 2), 1)
    ...
    TaskGoStraightToCoord(CORRUPT.getPlayerPed(), ao.x, ao.y, ao.z, 1.0, 5000, ap, 0.01)
...
function betChipsForNextHand(aH, av, aI, ah, aJ, aK)
    ...
    local aQ = vVar8 + aP
    SetEntityRotation(aO, aQ.x, aQ.y, aQ.z, 2, 1)
end
function getDealerGenderFromPed(U)
    local aR = "S_M_Y_Casino_01"
    local aS = "S_F_Y_Casino_01"
    if GetEntityModel(U) == aR then
        return "male"
    end
    return "female"
end
function getNewCardFromMachine(nextCard, Y, ab)
    local aT = CORRUPT.loadModel(nextCard)
    local aU = blackjack_func_399(blackjack_func_368(Y))
    local aw = CreateObjectNoOffset(aT, aU.x, aU.y, aU.z, false, false, 1)
    ...
    SetEntityVisible(aw, false, false)
    SetModelAsNoLongerNeeded(aT)
    local cardObjectOffsetRotation = blackjack_func_398(blackjack_func_368(Y))
    SetEntityCoordsNoOffset(aw, aU.x, aU.y, aU.z, 0, 0, 1)
    SetEntityRotation(aw, cardObjectOffsetRotation.x, cardObjectOffsetRotation.y, cardObjectOffsetRotation.z, 2, 1)
    return aw
end
function dealerGiveCards(Y, gender, U, cardObj)
    local aV = tostring(getLocalChairIdFromGlobalChairId(Y))
    TaskPlayAnim(
        U,
        "anim_casino_b@amb@casino@games@blackjack@dealer",
        gender .. "deal_card_player_0" .. aV,
        3.0,
        1.0,
        -1,
        2,
    ...
    PlayFacialAnim(U, "deal_card_player_0" .. aV .. "_facial")
    Wait(300)
    SetEntityVisible(cardObj, true, true)
    while not HasAnimEventFired(U, 585557868) do
        Wait(0)
    end
end
...
function declineCard()
    E = false
    local aX = blackjack_func_348(F)
    local aY = blackjack_func_215(F)
    local aZ = NetworkCreateSynchronisedScene(aX.x, aX.y, aX.z, aY.x, aY.y, aY.z, 2, 1, 0, 1065353216, 0, 1065353216)
    NetworkAddPedToSynchronisedScene(
        CORRUPT.getPlayerPed(),
        aZ,
        "anim_casino_b@amb@casino@games@blackjack@player",
...
function blackjack_func_398(a_)
    local vVar0 = vector3(0.0, 164.52, 11.5)
    return vector3(getTableHeading(a_), 0.0, 0.0) + vVar0
end
function blackjack_func_399(a_)
    local vVar0 = vector3(0.526, 0.571, 0.963)
    local b0, b1, b2 = getTableCoords(a_)
    return GetObjectOffsetFromCoords(b0, b1, b2, getTableHeading(a_), vVar0.x, vVar0.y, vVar0.z)
end
function ensureCardModelsLoaded()
    local b3 = 0
    while b3 < 52 do
        local b4 = 1
        local b5 = getCardFromNumber(b3, b4)
        CORRUPT.loadModel(b5)
        b3 = b3 + 1
    end
end
function ensureCardModelsReleased()
    ...
    while b3 < 52 do
        local b4 = 1
        local b5 = getCardFromNumber(b3, b4)
        SetModelAsNoLongerNeeded(b5)
        b3 = b3 + 1
    end
end
function blackjack_func_204(a_, b6, b7)
    if b7 then
        return vector3(getTableHeading(b6), 0.0, 0.0) + vector3(0, 0.061, -59.1316)
    else
        vVar0 = blackjack_func_215(a_)
        return vector3(vVar0.z, 0.0, 0.0) + vector3(-87.48, 0, -60.84)
    end
    return 0.0, 0.0, 0.0
end
function blackjack_func_205(a_, b6, b7)
    if b7 then
        local b8 = getTableCoords(b6)
        return GetObjectOffsetFromCoords(b8.x, b8.y, b8.z, getTableHeading(b6), -0.0094, -0.0611, 1.5098)
    else
        vVar0 = blackjack_func_215(a_)
        local b8 = blackjack_func_348(a_)
        return GetObjectOffsetFromCoords(b8.x, b8.y, b8.z, vVar0.z, 0.245, 0.0, 1.415)
    end
    return 0.0, 0.0, 0.0
end
function blackjack_func_216(a_, b6)
    local aq = blackjack_func_348(a_)
    local ar, as, at = blackjack_func_215(a_)
    vVar0 =
        GetAnimInitialOffsetRotation(
        "anim_casino_b@amb@casino@games@shared@player@",
        blackjack_func_213(b6),
        aq.x,
        aq.y,
        aq.z,
        ar,
...
function blackjack_func_217(a_, b9, b7)
    local vVar0 = {}
    if not IsEntityDead(a_) then
        vVar0 = GetEntityCoords(a_, 1)
    else
        vVar0 = GetEntityCoords(a_, 0)
    end
    return #(vVar0 - b9)
end
function blackjack_func_218(a_, b6)
    local aq = blackjack_func_348(a_)
    local ar, as, at = blackjack_func_215(a_)
    vVar0 =
...
function blackjack_func_213(ba)
    if ba == 0 then
...
function blackjack_func_348(a_)
    local tableId = blackjack_func_368(a_)
    local bb = G.blackjackTables[tableId].objectId
    if DoesEntityExist(bb) then
        local localChairId = getLocalChairIndexFromGlobalChairId(a_)
        localChairId = getInverseChairId(localChairId) + 1
        local bc = getBlackjackTableBoneIndex(localChairId)
        return GetEntityBonePosition_2(bb, bc)
    else
...
function blackjack_func_215(a_)
    local tableId = blackjack_func_368(a_)
    local bb = G.blackjackTables[tableId].objectId
    if DoesEntityExist(bb) then
        local localChairId = getLocalChairIndexFromGlobalChairId(a_)
        localChairId = getInverseChairId(localChairId) + 1
        return GetEntityBoneRotation(bb, getBlackjackTableBoneIndex(localChairId))
    else
        return vector3(0.0, 0.0, 0.0)
    end
end
...
function getLocalChairIdFromGlobalChairId(bd)
    if bd ~= -1 then
        return bd % 4 + 1
    else
        return 100
...
function getLocalChairIndexFromGlobalChairId(bd)
    if bd ~= -1 then
        return bd % 4
    else
        return 100
...
function getTableHeading(be)
    if G.blackjackTables[be] ~= nil then
        return G.blackjackTables[be].tableHeading
    else
        return 0.0
    end
end
function getTableCoords(be)
    if G.blackjackTables[be] ~= nil then
        return G.blackjackTables[be].tablePos.x, G.blackjackTables[be].tablePos.y, G.blackjackTables[be].tablePos.z
    else
        return 0.0, 0.0, 0.0
    end
end
function getCardFromNumber(a_, bf)
    if bf then
        if a_ == 1 then
...
function blackjack_func_377(a_, b6, b7)
    if b7 == 0 then
        if b6 == 0 then
            if a_ == 0 then
...
function func_376(a_, b6, b7, bg)
    local bh = vector3(0.0, 0.0, 0.0)
    if not b7 then
        if b6 == 0 then
            if a_ == 0 then
                bh = vector3(0.0, 0.0, 69.12)
            elseif a_ == 1 then
                bh = vector3(0.0, 0.0, 67.8)
            elseif a_ == 2 then
                bh = vector3(0.0, 0.0, 66.6)
            elseif a_ == 3 then
                bh = vector3(0.0, 0.0, 70.44)
            elseif a_ == 4 then
                bh = vector3(0.0, 0.0, 70.84)
            elseif a_ == 5 then
                bh = vector3(0.0, 0.0, 67.88)
            elseif a_ == 6 then
                bh = vector3(0.0, 0.0, 69.56)
            end
        elseif a_ == 1 then
            if a_ == 0 then
                bh = vector3(0.0, 0.0, 22.11)
            elseif a_ == 1 then
                bh = vector3(0.0, 0.0, 22.32)
            elseif a_ == 2 then
                bh = vector3(0.0, 0.0, 20.8)
            elseif a_ == 3 then
                bh = vector3(0.0, 0.0, 19.8)
            elseif a_ == 4 then
                bh = vector3(0.0, 0.0, 19.44)
            elseif a_ == 5 then
                bh = vector3(0.0, 0.0, 26.28)
            elseif a_ == 6 then
                bh = vector3(0.0, 0.0, 22.68)
            end
        elseif a_ == 2 then
            if a_ == 0 then
                bh = vector3(0.0, 0.0, -21.43)
            elseif a_ == 1 then
                bh = vector3(0.0, 0.0, -20.16)
            elseif a_ == 2 then
                bh = vector3(0.0, 0.0, -16.92)
            elseif a_ == 3 then
                bh = vector3(0.0, 0.0, -23.4)
            elseif a_ == 4 then
                bh = vector3(0.0, 0.0, -21.24)
            elseif a_ == 5 then
                bh = vector3(0.0, 0.0, -23.76)
            elseif a_ == 6 then
                bh = vector3(0.0, 0.0, -19.44)
            end
        elseif a_ == 3 then
            if a_ == 0 then
                bh = vector3(0.0, 0.0, -67.03)
            elseif a_ == 1 then
                bh = vector3(0.0, 0.0, -69.12)
            elseif a_ == 2 then
                bh = vector3(0.0, 0.0, -64.44)
            elseif a_ == 3 then
                bh = vector3(0.0, 0.0, -67.68)
            elseif a_ == 4 then
                bh = vector3(0.0, 0.0, -63.72)
            elseif a_ == 5 then
                bh = vector3(0.0, 0.0, -68.4)
            elseif a_ == 6 then
                bh = vector3(0.0, 0.0, -64.44)
            end
        end
        ...
        if b6 == 0 then
            if a_ == 0 then
                bh = vector3(0.0, 0.0, 68.57)
            elseif a_ == 1 then
                bh = vector3(0.0, 0.0, 67.52)
            elseif a_ == 2 then
                bh = vector3(0.0, 0.0, 67.76)
            elseif a_ == 3 then
                bh = vector3(0.0, 0.0, 67.04)
            elseif a_ == 4 then
                bh = vector3(0.0, 0.0, 68.84)
            elseif a_ == 5 then
                bh = vector3(0.0, 0.0, 65.96)
            elseif a_ == 6 then
                bh = vector3(0.0, 0.0, 67.76)
            end
        elseif b6 == 1 then
            if a_ == 0 then
                bh = vector3(0.0, 0.0, 22.11)
            elseif a_ == 1 then
                bh = vector3(0.0, 0.0, 22)
            elseif a_ == 2 then
                bh = vector3(0.0, 0.0, 24.44)
            elseif a_ == 3 then
                bh = vector3(0.0, 0.0, 21.08)
            elseif a_ == 4 then
                bh = vector3(0.0, 0.0, 25.96)
            elseif a_ == 5 then
                bh = vector3(0.0, 0.0, 26.16)
            elseif a_ == 6 then
                bh = vector3(0.0, 0.0, 28.76)
            end
        elseif b6 == 2 then
            if a_ == 0 then
                bh = vector3(0.0, 0.0, -14.04)
            elseif a_ == 1 then
                bh = vector3(0.0, 0.0, -15.48)
            elseif a_ == 2 then
                bh = vector3(0.0, 0.0, -16.56)
            elseif a_ == 3 then
                bh = vector3(0.0, 0.0, -15.84)
            elseif a_ == 4 then
                bh = vector3(0.0, 0.0, -16.92)
            elseif a_ == 5 then
                bh = vector3(0.0, 0.0, -14.4)
            elseif a_ == 6 then
                bh = vector3(0.0, 0.0, -14.28)
            end
        elseif b6 == 3 then
            if a_ == 0 then
                bh = vector3(0.0, 0.0, -67.03)
            elseif a_ == 1 then
                bh = vector3(0.0, 0.0, -67.6)
            elseif a_ == 2 then
                bh = vector3(0.0, 0.0, -69.4)
            elseif a_ == 3 then
                bh = vector3(0.0, 0.0, -69.04)
            elseif a_ == 4 then
                bh = vector3(0.0, 0.0, -68.68)
            elseif a_ == 5 then
                bh = vector3(0.0, 0.0, -66.16)
            elseif a_ == 6 then
                bh = vector3(0.0, 0.0, -63.28)
            end
        end
    end
    if bg then
        bh = vector3(bh.x, bh.y, bh.z + 90.0)
    end
    return bh
end
function getChipPropFromAmount(au)
    ...
    if au < 1000000 then
        ...
        for al, v in ipairs(denominations) do
            while au >= denominations[bi] do
                table.insert(chips, denominations[bi])
                au = au - denominations[bi]
            end
            bi = bi - 1
        end
...
function blackjack_func_374(a9, b6, Y, bg)
    fVar0 = 0.0
    vVar1 = vector3(0, 0, 0)
    if not bg then
        if a9 == 10 then
...
function blackjack_func_373(a_, b6, bk, bg)
    if not bg then
        if bk == 0 then
            if b6 == 0 then
...
function ButtonMessage(bl)
    BeginTextCommandScaleformString("STRING")
    AddTextComponentSubstringKeyboardDisplay(bl)
    EndTextCommandScaleformString()
end
function Button(bm)
    ScaleformMovieMethodAddParamPlayerNameString(bm)
end
function setupBlackjackInstructionalScaleform(bn)
    local bn = RequestScaleformMovie(bn)
    while not HasScaleformMovieLoaded(bn) do
        Citizen.Wait(0)
    end
    BeginScaleformMovieMethod(bn, "CLEAR_ALL")
    EndScaleformMovieMethod()
    BeginScaleformMovieMethod(bn, "SET_CLEAR_SPACE")
    ScaleformMovieMethodAddParamInt(200)
    EndScaleformMovieMethod()
    BeginScaleformMovieMethod(bn, "SET_DATA_SLOT")
    ...
    EndScaleformMovieMethod()
    BeginScaleformMovieMethod(bn, "DRAW_INSTRUCTIONAL_BUTTONS")
    EndScaleformMovieMethod()
    BeginScaleformMovieMethod(bn, "SET_BACKGROUND_COLOUR")
    ScaleformMovieMethodAddParamInt(0)
    ScaleformMovieMethodAddParamInt(0)
    ScaleformMovieMethodAddParamInt(0)
...
function setupBlackjackMidBetScaleform(bn)
    local bn = RequestScaleformMovie(bn)
    while not HasScaleformMovieLoaded(bn) do
        Citizen.Wait(0)
    end
...
function getDealerIdFromEntity(Q)
    local bo = nil
    local bp = 10000
    local bq = GetEntityCoords(Q)
    for al, v in pairs(G.blackjackTables) do
        local br = v.dealerPos
        if #(bq - br) < bp then
            bo = al
            bp = #(bq - br)
        end
...
function setBlackjackDealerPedVoiceGroup(bs, U)
    if bs == 0 then
...
function setBlackjackDealerClothes(bs, U)
    if bs == 0 then
