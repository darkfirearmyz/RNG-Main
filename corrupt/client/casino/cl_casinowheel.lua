...
RegisterNetEvent("CORRUPT:frameworkCleanup")
AddEventHandler(
    "CORRUPT:frameworkCleanup",
    function()
        DeleteEntity(b)
    end
)
        ...
        RageUI.IsVisible(
            ...
            function()
                RageUI.ButtonWithStyle(
                    "Spin the wheel of CORRUPT (50,000 Chips)",
                    "",
                    {RightLabel = "→→→"},
                    true,
                    function(e, f, g)
                        if g then
                            TriggerServerEvent("CORRUPT:requestSpinLuckyWheel")
                        end
                    end
                )
            end,
...
Citizen.CreateThread(
    function()
        ...
        local i = CORRUPT.loadModel("vw_prop_vw_luckywheel_02a")
        local j = CORRUPT.loadModel(c)
...
function goToWheel()
    if not d then
        ...
        local D = false
        while not D do
            local E = GetEntityCoords(CORRUPT.getPlayerPed())
        ...
        SetEntityHeading(CORRUPT.getPlayerPed(), 10.9754)
        Wait(2000)
    ...
    function(D, E)
        if E then
            ...
            for K, L in pairs(a) do
                ...
                CORRUPT.createArea("corruptwheel_" .. K, L.entryPosition, 1.5, 6, F, H, I, {})
            end
        end
    end
)
RegisterNetEvent("CORRUPT:syncLuckyWheel")
AddEventHandler(
    "CORRUPT:syncLuckyWheel",
    function(M)
        Wait(900)
        actuallyRollWheel(M)
    end
)
RegisterNetEvent("CORRUPT:spinLuckyWheel")
AddEventHandler(
    "CORRUPT:spinLuckyWheel",
    function()
        goToWheel()
    end
)
RegisterNetEvent("CORRUPT:spinLuckyWheelReaction")
AddEventHandler(
    "CORRUPT:spinLuckyWheelReaction",
    function(N)
        local y = CORRUPT.getPlayerPed()
