...
RMenu:Get("CORRUPTjackpot", "instructions"):SetSubtitle("JACKPOT")
RageUI.CreateWhile(
    1.0,
    RMenu:Get("CORRUPTjackpot", "instructions"),
    nil,
    function()
        RageUI.IsVisible(
            RMenu:Get("CORRUPTjackpot", "instructions"),
    ...
    function()
        while true do
            if insideDiamondCasino then
                b = 1000
                a = -1
                local y = GetEntityCoords(CORRUPT.getPlayerPed())
                for z = 0, 15, 1 do
                    local A = t[z]
                    local B = #(y - A)
                    if B < b then
...
RegisterNetEvent(
    "CORRUPT:updatePlayerWinChance",
    function(F)
        k = F
    end
)
RegisterNetEvent(
    "CORRUPT:updateTotalPot",
    function(G)
        r = G
    end
)
function goToJackpotChair(E)
    ...
    PlayFacialAnim(CORRUPT.getPlayerPed(), "enter_left_readyidle_facial", H)
    RemoveAnimDict(H)
    local O
    local P
                ...
                if j then
                    ...
                    if u ~= nil then
                        for R, S in pairs(u) do
                            if S.user_id == CORRUPT.getUserId() then
                                DrawRect(
                                    0.72,
                                    0.82,
                                    0.115,
        ...
        while true do
            if f then
                ...
                if IsControlJustPressed(0, 201) then
                    if h then
                        TriggerServerEvent("CORRUPT:setJackpotBet", p)
                    else
...
function showHowToJackpot(a4)
    RageUI.CloseAll()
    if a ~= -1 then
        RageUI.Visible(RMenu:Get("CORRUPTjackpot", "instructions"), a4)
    end
end
Citizen.CreateThread(
    function()
        while true do
            if a ~= -1 and b < 2 then
                if IsControlJustPressed(0, 38) then
                    if d[a] == false then
                        TriggerServerEvent("CORRUPT:requestSitAtJackpot", a)
                    end
                end
            end
            Wait(0)
...
AddEventHandler(
    ...
    function(a5, a6)
        if a6 then
            TriggerServerEvent("CORRUPT:requestJackpotChairData")
        end
    end
)
RegisterNetEvent(
    "CORRUPT:sendJackpotChairData",
    function(a7)
        d = a7
    end
)
        ...
        while true do
            if sittingAtInsideTrack and f then
                ...
                if IsControlJustPressed(0, 202) and not g then
                    ...
                    PlayFacialAnim(CORRUPT.getPlayerPed(), "exit_left_facial", H)
                    SetTimeout(
                        2500,
                        function()
                    ...
                    f = false
                    showHowToJackpot(false)
                    TriggerServerEvent("CORRUPT:leaveJackpotChair")
                    e = -1
                    SetTimeout(
                        5000,
                        function()
...
function getTrackGenderAnimDict()
    local aw = GetEntityModel(CORRUPT.getPlayerPed())
...
RegisterNetEvent(
    "CORRUPT:newJackpotBet",
    function(ax)
        table.insert(u, ax)
    end
)
RegisterNetEvent(
    "CORRUPT:cleanupJackpot",
    function()
        u = {}
        if e ~= -1 then
            f = true
            ...
            NetworkAddPedToSynchronisedScene(CORRUPT.getPlayerPed(), ay, H, I, 2.0, -2.0, 5, 0, 1148846080, 0)
