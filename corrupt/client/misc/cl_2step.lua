local a = {twoStepControl = 137, TwoStepCars = {"rsqr8", "xxxx"}}
local b = {}
local c
local d = false
local e = false
Citizen.CreateThread(
    function()
        while true do
            local f = CORRUPT.getPlayerPed()
            local g = GetVehiclePedIsIn(CORRUPT.getPlayerPed(), false)
            if g ~= 0 and GetPedInVehicleSeat(g, -1) == f then
                local h, i = tvRP.getVehicleInfos(g)
                if h == CORRUPT.getUserId() then
                    if b[i] ~= nil and b[i] ~= 0 then
                        c = i
                        d = true
                        while GetVehiclePedIsIn(CORRUPT.getPlayerPed(), false) ~= 0 and e == false do
                            Wait(100)
                        end
                        e = false
                        d = false
