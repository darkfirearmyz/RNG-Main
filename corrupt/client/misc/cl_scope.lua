local function a()
    local b = GetResourceKvpString("corrupt_scope_horizontal")
    if b == nil or b == "" then
        return 50
    else
        return tonumber(b)
...
local function f()
    local b = GetResourceKvpString("corrupt_scope_vertical")
    if b == nil or b == "" then
        return 30
    else
        return tonumber(b)
...
local i = GetResourceKvpString("corrupt_scope_enabled") == "true"
local j = false
RMenu.Add(
    "scope",
    "main",
            ...
            function()
                ...
                RageUI.List(
                    ...
                    function(k, l, m, n)
                        d = n
                        if l then
                            SetResourceKvp("corrupt_scope_horizontal", tostring(n))
                        end
                    end
                )
                RageUI.List(
                    ...
                    function(k, l, m, n)
                        h = n
                        if l then
                            SetResourceKvp("corrupt_scope_vertical", tostring(n))
                        end
                    end
                )
                RageUI.Checkbox(
                    ...
                    function(k, m, l, o)
                        i = o
                        if l then
                            SetResourceKvp("corrupt_scope_enabled", tostring(o))
                        end
                    end
                )
            end,
...
function CORRUPT.doesCurrentWeaponHaveScope()
    local p = PlayerPedId()
...
Citizen.CreateThread(
    function()
        while true do
            Citizen.Wait(0)
            if i and (j or CORRUPT.doesCurrentWeaponHaveScope()) then
                if j then
