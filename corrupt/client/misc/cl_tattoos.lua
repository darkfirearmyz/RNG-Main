    ...
    function(D, E)
        if E then
            local F = function()
                ...
                c = true
                i = deepcopy(h)
                j = tvRP.getCustomization()
                RageUI.Visible(RMenu:Get("corrupttattoos", "mainMenu"), true)
            end
            local G = function()
                ...
                RageUI.Visible(RMenu:Get("corrupttattoos", "mainMenu"), false)
                tvRP.setCustomization(j)
                j = nil
                ClearPedDecorations(CORRUPT.getPlayerPed())
                for H = 1, #i, 1 do
            ...
            local I = function()
                ...
                if RageUI.Visible(RMenu:Get("corrupttattoos", "mainMenu")) then
            ...
            for B, J in pairs(f) do
                ...
                Wait(100)
                CORRUPT.createArea("tattoParlour_" .. B, J, 1.5, 6, F, G, I)
                tvRP.addBlip(J.x, J.y, J.z, 75, 0, "Tattoo Parlour")
                tvRP.addMarker(
                    J.x,
                    J.y,
                    J.z,
...
Citizen.CreateThread(
    function()
        ...
        while true do
            Wait(500)
            if tvRP.isInComa() and not P then
                P = true
            end
            if not tvRP.isInComa() and P then
                Wait(5000)
                TriggerServerEvent("CORRUPT:getPlayerTattoos")
                P = false
            end
...
RageUI.CreateWhile(
    1.0,
    RMenu:Get("corrupttattoos", "mainMenu"),
    nil,
    function()
        RageUI.IsVisible(
            RMenu:Get("corrupttattoos", "mainMenu"),
                        ...
                        function(U, V, W, X)
                            if V then
                                ...
                                if c and not l and IsControlJustReleased(0, 178) then
                                    ...
                                    TriggerServerEvent("CORRUPT:saveTattoos", i, 1000)
                                end
                            end
                            if W then
...
function drawTattoo(_, a0, a1)
    SetEntityHeading(CORRUPT.getPlayerPed(), 297.7296)
    local C = CORRUPT.getPlayerPed()
    local a2 = false
    if _ ~= nil and a0 ~= nil and a1 ~= nil then
        if p then
            ...
            if GetEntityModel(CORRUPT.getPlayerPed()) == -1667301416 then
                CORRUPT.loadCustomisationPreset("TattooMale")
            else
                CORRUPT.loadCustomisationPreset("TattooFemale")
            end
            ClearPedDecorations(CORRUPT.getPlayerPed())
            for H = 1, #i, 1 do
                AddPedDecorationFromHashes(
            ...
            AddPedDecorationFromHashes(CORRUPT.getPlayerPed(), GetHashKey(a0), GetHashKey(e[a0][_].nameHash))
            if not DoesCamExist(b) then
                b = CreateCam("DEFAULT_SCRIPTED_CAMERA", true)
                SetCamCoord(b, GetEntityCoords(CORRUPT.getPlayerPed()))
                SetCamRot(b, 0.0, 0.0, 0.0)
                SetCamActive(b, true)
                RenderScriptCams(true, false, 0, true, true)
                SetCamCoord(b, GetEntityCoords(CORRUPT.getPlayerPed()))
            end
            local S, a3, a4 = table.unpack(GetEntityCoords(CORRUPT.getPlayerPed()))
