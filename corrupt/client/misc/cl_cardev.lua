...
RageUI.CreateWhile(
    ...
    function()
        RageUI.IsVisible(
            ...
            true,
            function()
                if b and CORRUPT.getPlayerBucket() == 333 then
                        ...
                        function(h, i, j)
                            if j then
                                CORRUPT.clientPrompt(
                                    ...
                                    "",
                                    function(k)
                                        if k ~= "" and CORRUPT.getPlayerBucket() == 333 then
                                            local l = CORRUPT.loadModel(k)
                                            TriggerServerEvent("CORRUPT:logVehicleSpawn", k, "/cardev")
                                            ...
                                            SetPedIntoVehicle(CORRUPT.getPlayerPed(), n, -1)
                    ...
                    RageUI.Checkbox(
                        ...
                        function()
                            b = true
                            TriggerServerEvent("CORRUPT:setCarDevMode", b)
                        end,
                        function()
                            b = false
                            TriggerServerEvent("CORRUPT:setCarDevMode", b)
        ...
        RageUI.IsVisible(
            ...
            function()
                for w, x in pairs(f) do
                    if GetNumVehicleMods(CORRUPT.getPlayerVehicle(), w) > 0 then
                        RageUI.ButtonWithStyle(
                            x,
                            "",
                            {RightLabel = "→→→"},
        ...
        RageUI.IsVisible(
            ...
            true,
            function()
                if GetNumVehicleMods(CORRUPT.getPlayerVehicle(), selectedModType) == 0 then
                    RageUI.Text({message = "No available mod indexes for this mod type for this vehicle."})
                else
                    for w = 0, GetNumVehicleMods(CORRUPT.getPlayerVehicle(), selectedModType) do
                        RageUI.ButtonWithStyle(
                            "Mod " .. w,
                            "",
                            {RightLabel = "→→→"},
                            ...
                            function(h, i, j)
                                if j then
                                    SetVehicleModKit(CORRUPT.getPlayerVehicle(), 0)
                                    SetVehicleMod(CORRUPT.getPlayerVehicle(), selectedModType, w)
                                end
                            end,
                            RMenu:Get("cardev", "vehiclemodindexes")
                        )
                                ...
                                RageUI.ButtonWithStyle(
                                    ...
                                    function(h, i, j)
                                        if j then
                                            ...
                                            TriggerEvent("CORRUPT:staffFixVehicle")
                                        end
                                    end
                                )
                            end
            ...
            function()
                for z, A in pairs(g) do
                    RageUI.ButtonWithStyle(
                        ...
                        function(h, i, j)
                            if j then
                                SetVehicleColours(CORRUPT.getPlayerVehicle(), A.colorindex, A.colorindex)
                            end
                        end
                    )
                end
...
RegisterCommand(
    "cardev",
    function(C)
        if a and not CORRUPT.isPurge() then
            B()
        end
    end
)
...
local function ae()
    if not b or CORRUPT.getPlayerBucket() ~= 333 then
        if J then
            ac(false)
        end
        return
    ...
    L()
    local af = CORRUPT.getFontId("Akrobat-ExtraBold")
    ...
    local t = CORRUPT.getPlayerVehicle()
