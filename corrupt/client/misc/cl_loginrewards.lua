...
RageUI.CreateWhile(
    1.0,
    RMenu:Get("loginrewards", "mainmenu"),
    nil,
    function()
        RageUI.IsVisible(
            RMenu:Get("loginrewards", "mainmenu"),
            ...
            function()
                ...
                RageUI.ButtonWithStyle(
                    ...
                    function(p, p, u)
                        if u then
                            TriggerServerEvent("CORRUPT:claimNextLoginReward")
                            b = b + 1
...
RegisterNetEvent(
    "CORRUPT:setDailyRewardInfo",
    function(w, x, y)
        b = w
        c = GetGameTimer() + (86400 - (y - x)) * 1000
        if w == 0 then
            Citizen.Wait(20000)
            notify("~y~You have not started your daily reward. Do /dailyrewards to start now!")
        end
    end
)
RegisterCommand(
...
Citizen.CreateThread(
    function()
        ...
        while true do
            if r() and b > 0 then
                notify("~g~You can claim your next daily reward at /dailyrewards!")
                break
            end
            Citizen.Wait(6000)
        end
