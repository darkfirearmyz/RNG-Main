...
local function c()
    if GetEntityHealth(CORRUPT.getPlayerPed()) <= 102 then
        tvRP.notify("~r~You try to move but you remember you're dead")
        return true
    elseif IsEntityDead(CORRUPT.getPlayerPed()) then
        tvRP.notify("~r~You try to move but you remember you're dead")
        return true
    else
        return false
...
RegisterCommand(
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and CORRUPT.canAnim() and not IsEntityDead(h) and d() then
            CORRUPT.allowweapon(0x88C78EB7, 1)
...
RegisterCommand(
    ...
    function(f, g)
        local h = CORRUPT.getPlayerPed()
        if DoesEntityExist(h) and CORRUPT.canAnim() and not IsEntityDead(h) and d() then
            CORRUPT.allowweapon(0x01B79F17, 1)
...
function surrender()
    ...
    if DoesEntityExist(h) and CORRUPT.canAnim() and not IsEntityDead(h) and d() then
        CORRUPT.loadAnimDict("random@arrests")
        CORRUPT.loadAnimDict("random@arrests@busted")
...
function tvRP.isPlayerSurrendered()
...
function tvRP.isPlayerSurrenderedNoProgressBar()
    return j()
end
local u
RegisterNetEvent(
    "CORRUPT:startRobbingPlayer",
    function()
        u = GetGameTimer()
    end
)
RegisterNetEvent(
    "CORRUPT:endRobbingPlayer",
    function()
        u = nil
    end
)
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if B then
                ...
                tvRP.notify("Press ~r~[E]~w~ to take a picture!")
                B = true
            end
        end
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if y then
                ...
                Wait(120)
                tvRP.notify("Press ~r~[E]~w~ to take a toke!")
                y = true
            end
        end
        while y do
            ...
            if IsControlJustPressed(0, 38) then
                ...
                Wait(100)
                tvRP.notify("You take a huge rip!")
                Wait(7250)
            end
        end
    end,
    ...
    function(f, g)
        ...
        if DoesEntityExist(h) and not IsEntityDead(h) then
            ...
            if v then
                ...
                Wait(120)
                tvRP.notify("Press ~r~[E]~w~ to wave the wand.")
                v = true
            end
        end
            ...
            if
             ...
             then
                if not Y and globalNHSOnDuty then
                    ...
                    Y = false
                    DetachEntity(CORRUPT.getPlayerPed(), false, false)
                    ClearPedSecondaryTask(CORRUPT.getPlayerPed())
                    local a5 = GetClosestPlayer(3)
                    local a7 = GetPlayerServerId(a5)
                    if a7 ~= 0 then
                        TriggerServerEvent("CORRUPT:animationSyncStop", a7)
                    end
                end
                Wait(250)
            end
...
RegisterNetEvent(
    ...
    function()
        ...
        SetPedMotionBlur(CORRUPT.getPlayerPed(), true)
        SetPedMovementClipset(CORRUPT.getPlayerPed(), "move_m@drunk@verydrunk", true)
        RemoveAnimSet("move_m@drunk@verydrunk")
        SetPedIsDrunk(CORRUPT.getPlayerPed(), true)
        DoScreenFadeIn(1000)
        ClearPedSecondaryTask(CORRUPT.getPlayerPed())
        DeleteObject(prop)
        Wait(60000)
        ...
        ResetPedMovementClipset(CORRUPT.getPlayerPed(), 0)
        SetPedIsDrunk(CORRUPT.getPlayerPed(), false)
        SetPedMotionBlur(CORRUPT.getPlayerPed(), false)
        RemoveAnimDict(H)
    end
)
RegisterNetEvent(
...
RegisterNetEvent(
    ...
    function(ai)
        ...
        CORRUPT.loadAnimDict("misshair_shop@hair_dressers")
        ...
        CORRUPT.loadModel("prop_clippers_01")
...
RegisterNetEvent(
    ...
    function(au)
        ...
        local aw = CORRUPT.getPlayerCoords()
...
CORRUPT.createThreadOnTick(az)
RegisterNetEvent(
    "CORRUPT:playArmourApplyAnim",
    function()
        CORRUPT.loadAnimDict("clothingtie")
        tvRP.setCanAnim(false)
