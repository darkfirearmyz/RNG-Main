    ...
    if not a.InProgress and u then
        ...
        if i then
            ...
            if w ~= -1 then
                ...
                if x then
                    CORRUPT.setWeapon(PlayerPedId(), foundWeapon, true)
                    a.InProgress = true
                    a.startTime = GetGameTimer()
                    a.targetSrc = w
                    TriggerServerEvent("CORRUPT:takeHostageSync", w)
                    a.type = "agressor"
                else
...
RegisterNetEvent(
    "CORRUPT:takeHostageReleaseHostage",
    function()
        a.InProgress = false
        a.type = ""
        DetachEntity(PlayerPedId(), true, false)
...
RegisterNetEvent(
    "CORRUPT:takeHostageKillHostage",
    function()
        a.InProgress = false
        a.type = ""
        CORRUPT.setHealth(0)
        ...
        while true do
            if a.type == "agressor" then
                ...
                if IsEntityDead(PlayerPedId()) then
                    ...
                    RemoveAnimDict("reaction@shove")
                    TriggerServerEvent("CORRUPT:takeHostageReleaseHostage", a.targetSrc)
                end
                ...
                elseif IsDisabledControlJustPressed(0, 74) and GetGameTimer() - a.startTime > 4000 then
                    ...
                    RemoveAnimDict("anim@gangops@hostage@")
                    TriggerServerEvent("CORRUPT:takeHostageKillHostage", a.targetSrc)
                    TriggerServerEvent("CORRUPT:takeHostageStop", a.targetSrc)
                    Wait(100)
...
function CORRUPT.takeHostageInProgress()
    return a.InProgress
end
