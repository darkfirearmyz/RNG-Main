...
Citizen.CreateThread(
    function()
        while true do
            local e = CORRUPT.getPlayerVehicle()
            if e ~= 0 and GetPedInVehicleSeat(e, -1) == CORRUPT.getPlayerPed() then
                local f = GetVehicleEngineHealth(e)
                if f <= b and a[e] == nil then
                    local g = GetVehicleClass(e)
                    if g ~= 8 and g ~= 16 and g ~= 15 and g ~= 13 then
                        TriggerServerEvent("CORRUPT:playNuiSound", "airbag", 15.0, soundEventCode)
                        createAirbags(e)
                    end
                else
                    if a[e] == nil then
                        d = GetPedArmour(CORRUPT.getPlayerPed())
                    end
                end
            else
                d = 0
...
function createAirbags(e)
    ...
    if d > GetPedArmour(CORRUPT.getPlayerPed()) then
        CORRUPT.setArmour(d)
    end
    d = 0
    SetTimeout(
