local a = {
    ...
    [74] = {
        id = 74,
        position = vector3(-2150.8879394531,5190.158203125,15.715698242188),
        name = "VIP Island",
        destination = {76, 75},
    },
    [75] = {
        id = 75,
        position = vector3(-746.45275878906,5807.3276367188,17.602905273438),
        name = "Paleto Lodges",
        destination = {76, 74}
    },
    [76] = {
        id = 76,
        position = vector3(-2334.3955078125,266.017578125,169.58825683594),
        name = "Kortz",
        destination = {74, 75}
    },
    [95] = {
...
RMenu.Add(
    "vkint",
    "mainmenu",
    RageUI.CreateMenu("Teleporter", "Main Menu", CORRUPT.getRageUIMenuWidth(), CORRUPT.getRageUIMenuHeight())
)
local b = nil
local function c(d)
    if tvRP.getPlayerCombatTimer() > 0 then
        tvRP.notify("You can not teleport whilst you have a combat timer.")
        return
    end
...
Citizen.CreateThread(
    function()
        for f, d in pairs(a) do
            tvRP.addMarker(d.position.x, d.position.y, d.position.z - 1, 1.0, 1.0, 1.0, 255, 255, 255, 170, 50, 27)
            CORRUPT.createArea(
                "vkint_" .. d.id,
                d.position,
                2.0,
