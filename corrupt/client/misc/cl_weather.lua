...
function CORRUPT.setTime(f, g, h)
    b = {h = f, m = g, s = h}
end
function CORRUPT.overrideTime(f, g, h)
    b.h = f
    b.m = g
    b.s = h
    c = true
end
function CORRUPT.cancelOverrideTimeWeather()
    c = false
    TriggerServerEvent("CORRUPT:requestTimeWeatherSync")
end
function CORRUPT.setWeather(i)
    a = i
end
...
CORRUPT.createThreadOnTick(func_manageTimeAndWeather)
RegisterNetEvent(
    "CORRUPT:setTime",
    function(f, g, h)
        CORRUPT.setTime(f, g, h)
    end
)
RegisterNetEvent(
    "CORRUPT:setWeather",
    function(i)
        CORRUPT.setWeather(i)
    end
)
...
function voteWeather(C)
    if j then
        if not k then
            TriggerServerEvent("CORRUPT:vote", C)
            k = true
            tvRP.notify("~g~Vote sent!")
        else
            tvRP.notify("You have already voted!")
        end
    else
        tvRP.notify("Vote not in progress, start a vote with /voteweather!")
    end
end
function resetVotes()
    m = {
...
RegisterNetEvent(
    "CORRUPT:startWeatherVote",
    function()
        j = true
        tvRP.notify("~g~Weather vote has started! Type /[weather] e.g /snow or /rain to vote.")
    end
)
AddEventHandler(
    "playerSpawned",
