...
local o = json.decode(GetResourceKvpString("corrupt_gang_inv_colour")) or {r = 0, g = 50, b = 142}
RegisterNetEvent(
    "CORRUPT:addEmergencyCall",
    function(o, p, q, r, s, t)
        local u = 0
        if t == "admin" and CORRUPT.getStaffLevel() > 0 then
            tvRP.notify("~b~Admin ticket received.")
            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
            local v = #(r - GetEntityCoords(CORRUPT.getPlayerPed()))
            table.insert(d, {o, p, q, v, s, t, u})
            j()
            e[o] = r
        elseif t == "met" and CORRUPT.globalOnPoliceDuty() then
            ...
            j()
            e[o] = r
        elseif t == "nhs" and CORRUPT.globalNHSOnDuty() then
            ...
            if a then
                ...
                if CursorInArea(0.4234, 0.4614, 0.9444, 0.9759) then
                    ...
                    if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                        PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                        if h ~= nil then
                            TriggerServerEvent("CORRUPT:TakeTicket", h)
                            a = not a
                            ...
                            if C ~= nil then
                                TriggerServerEvent("CORRUPT:TakeTicket", C)
                                a = not a
                                SetNewWaypoint(e[h].x, e[h].y)
                                TriggerEvent("CORRUPT:showHUD", true)
                                globalHideEmergencyCallUI = false
...
local function u()
    if IsUsingKeyboard(2) and (CORRUPT.getStaffLevel() > 0 or globalOnPoliceDuty or globalNHSOnDuty or globalOnPrisonDuty) then
        a = not a
        if not a then
            TriggerEvent("CORRUPT:showHUD", true)
            globalHideEmergencyCallUI = false
