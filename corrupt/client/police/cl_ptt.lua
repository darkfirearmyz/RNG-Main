local a = false
function func_ptt()
    if CORRUPT.getRadioAnim() and CORRUPT.canAnim() and not noclipActive and CORRUPT.isInRadioChannel() then
        local b = CORRUPT.getPlayerPed()
        local c = CORRUPT.getPlayerId()
        if a then
...
CORRUPT.createThreadOnTick(func_ptt)
