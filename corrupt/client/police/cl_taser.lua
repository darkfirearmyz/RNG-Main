...
RegisterNetEvent(
    "CORRUPT:receiveActivation",
    function()
        if not tvRP.isStaffedOn() then
            displayTaserNotification("You are currently being ~b~activated~w~.")
            SetPedToRagdoll(CORRUPT.getPlayerPed(), 5000, 5000, 0, 0, 0, 0)
        end
    end
)
RegisterNetEvent(
    "CORRUPT:arcSuccessful",
    function()
        ...
        TriggerServerEvent("CORRUPT:playTaserSound", GetEntityCoords(CORRUPT.getPlayerPed()), "arcsound")
        b = false
        SetTimeout(
            4000,
            function()
...
RegisterNetEvent(
    ...
    function(f, g)
        local h = GetEntityCoords(CORRUPT.getPlayerPed(), true)
        local i = #(h - f)
        if i <= 15 then
...
RegisterNetEvent(
    "CORRUPT:reloadTaser",
    function(j)
        if GetSelectedPedWeapon(CORRUPT.getPlayerPed()) == GetHashKey(a.hash) then
            MakePedReload(CORRUPT.getPlayerPed())
            if a.cartridges == 0 then
                a.cartridges = 2
                if a.reactivate1.active then
                    TriggerServerEvent("CORRUPT:barbsNoLongerServer", a.reactivate1.serverId)
                end
                if a.reactivate2.active then
                    TriggerServerEvent("CORRUPT:barbsNoLongerServer", a.reactivate2.serverId)
                end
                a.reactivate1 = {active = false, ped = 0, serverId = 0}
                a.reactivate2 = {active = false, ped = 0, serverId = 0}
        ...
        if k.weapon == GetHashKey(a.hash) then
            if IsControlJustPressed(1, 308) then
                if a.reactivate1.active == true then
                    ...
                    TriggerServerEvent("CORRUPT:playTaserSound", k.playerCoords, "reactivate")
                    TriggerServerEvent("CORRUPT:reactivatePed", a.reactivate1.serverId)
                end
            elseif IsControlJustPressed(1, 307) then
                if a.reactivate2.active == true then
                    ...
                    TriggerServerEvent("CORRUPT:playTaserSound", k.playerCoords, "reactivate")
                    TriggerServerEvent("CORRUPT:reactivatePed", a.reactivate2.serverId)
                end
            elseif IsControlJustPressed(1, 121) and b then
                TriggerServerEvent("CORRUPT:arcTaser")
            end
            if a.cartridges < 1 then
                ...
                if IsPedShooting(k.playerPed) then
                    TriggerServerEvent("CORRUPT:playTaserSound", k.playerCoords, "taser")
                    a.cartridges = a.cartridges - 1
                    local l, m = GetEntityPlayerIsFreeAimingAt(k.playerId)
                    if l and IsEntityAPed(k.playerPed) then
                        local n = CORRUPT.getPedServerId(m)
                        TriggerServerEvent("CORRUPT:receiveBarbsServer", n)
                        if n ~= nil then
                            if a.cartridges == 1 then
...
CORRUPT.createThreadOnTick(func_taserHandler)
...
Citizen.CreateThread(
    function()
        SetPedMinGroundTimeForStungun(CORRUPT.getPlayerPed(), 15000)
        while true do
            if IsPedBeingStunned(CORRUPT.getPlayerPed()) then
                stunGun()
            end
            Wait(100)
        end
            ...
            if c then
                while c do
                    ...
                    if IsControlJustPressed(1, 38) then
                        c = false
                        if d and d > 0 then
                            TriggerServerEvent("CORRUPT:barbsRippedOutServer", d)
                        end
                        d = 0
...
function CORRUPT.setStunExitsEarly()
    p = true
end
