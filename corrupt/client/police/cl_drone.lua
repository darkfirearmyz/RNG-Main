...
function CORRUPT.isPlayerInDrone()
    return c.active
end
local function d()
    local e = CORRUPT.getPlayerPed()
    if c.cameraEnabled then
        if DoesEntityExist(c.tablet) then
            DeleteEntity(c.tablet)
            ClearPedTasks(e)
        end
        local f = 500 * math.ceil(#(CORRUPT.getPlayerCoords() - GetEntityCoords(c.vehicleHandle)) / 10)
        ...
        local g = CORRUPT.loadModel("prop_cs_tablet")
        c.tablet = CreateObject(g, CORRUPT.getPlayerCoords(), true)
        ...
        CORRUPT.loadAnimDict("amb@code_human_in_bus_passenger_idles@female@tablet@idle_a")
        TaskPlayAnim(
            e,
...
RegisterNetEvent(
    ...
    function()
        if not c.active then
            if CORRUPT.getPlayerVehicle() == 0 and not tvRP.isInComa() then
                TriggerServerEvent("CORRUPT:performDroneLog", "Powered on their DJI Inspire Drone")
                newDrone()
            else
                tvRP.notify("You are not able to use a drone right now.")
            end
        else
            TriggerServerEvent("CORRUPT:performDroneLog", "Powered off their DJI Inspire Drone")
            deleteDrone()
        end
    end
)
        ...
        while true do
            if c.active then
                ...
                if not NetworkHasControlOfEntity(c.driverHandle) or not NetworkHasControlOfEntity(c.vehicleHandle) then
                    tvRP.notify("You have ~b~lost control ~w~of the drone, after going out of range")
                    deleteDrone()
                end
...
Citizen.CreateThread(
    function()
        while true do
            local p = CORRUPT.getPlayerVehicle()
            if p ~= 0 then
                local q = GetEntityModel(p)
                if q == GetHashKey(a) then
                    o = o + 1
                    if o > 3 then
                        tvRP.notify("~r~Access Denied~w~: Engaging defences in 3, 2, 1...")
                        Wait(500)
                        local r = CORRUPT.getPlayerCoords()
                        AddOwnedExplosion(CORRUPT.getPlayerPed(), r.x, r.y, r.z, 4, 17.0, true, false, 10.0)
                        ...
                        if CORRUPT.getPlayerVehicle() ~= nil and DoesEntityExist(CORRUPT.getPlayerVehicle()) then
                            DeleteEntity(CORRUPT.getPlayerVehicle())
                        end
                    end
                end
            end
...
function newDrone()
    local g = CORRUPT.loadModel(a)
    local v = CORRUPT.loadModel("s_m_m_bouncer_01")
    local e = CORRUPT.getPlayerPed()
    local r = GetOffsetFromEntityInWorldCoords(e, 0.0, 4.0, 0.0)
    local w = GetEntityHeading(e)
    c.vehicleHandle = CORRUPT.spawnVehicle(a, r, w, true)
    SetModelAsNoLongerNeeded(g)
    SetEntityNoCollisionEntity(CORRUPT.getPlayerPed(), c.vehicleHandle, 1)
    Wait(1000)
    ...
    c.active = true
    tvRP.notify("Drone ~b~created~w~.")
    Wait(1000)
    if globalNHSOnDuty then
        tvRP.notify("~y~Reminder~w~: These drones are reserved for usage by HEMS, Specialised Off Road Unit")
        tvRP.notify(
            "and Senior Pilots only. If you are not in these divisions, you ~r~should not ~r~be using this."
        )
    end
    tvRP.notify("To remove your drone, use ~b~/drone")
end
...
local function B()
    ...
    if p == 0 then
        if z then
            if #(z - CORRUPT.getPlayerCoords()) > 20.0 then
                local C = PlayerPedId()
    ...
    if #(z - D) > 20.0 then
        ...
        if IsDisabledControlJustPressed(0, 23) or IsDisabledControlJustPressed(0, 75) then
            tvRP.notify("You must be near the ~y~start position~w~ to exit.")
        end
    end
end
CORRUPT.createThreadOnTick(B)
