    ...
    function(o, p)
        if c ~= "" then
            ...
            if table.count(p) ~= 0 then
                q = table.concat(p, " ")
                TriggerServerEvent("CORRUPT:flagVehicleAnpr", string.upper(c), q)
            else
                tvRP.notify("Error~w~: You must provide a valid flag reason.")
            end
        end
    end
)
RegisterCommand(
    ...
    function(o, p)
        if d ~= "" then
            ...
            if table.count(p) ~= 0 then
                q = table.concat(p, " ")
                TriggerServerEvent("CORRUPT:flagVehicleAnpr", string.upper(d), q)
            else
                tvRP.notify("Error~w~: You must provide a valid flag reason.")
            end
        end
...
CORRUPT.createThreadOnTick(func_drawFullAnpr)
function anprGetVehicle(u)
    ...
    local w = GetVehiclePedIsIn(CORRUPT.getPlayerPed(), false)
    ...
    if B > 0 and IsEntityAVehicle(B) then
        ...
        if C ~= nil then
            if m[C] ~= nil then
                ...
                tvRP.notify(string.format("ANPR Alert~w~: Plate ~b~%s ~w~is flagged for:~b~~n~%s", C, D))
                n = true
                SetTimeout(
                    10000,
                    function()
...
function isPedInPoliceVehicle()
    if IsPedInAnyVehicle(CORRUPT.getPlayerPed(), false) then
        if GetVehicleClass(CORRUPT.getPlayerVehicle()) == 18 and globalOnPoliceDuty then
            return true
        end
    end
    return false
