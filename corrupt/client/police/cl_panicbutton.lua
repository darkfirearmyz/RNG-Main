...
RegisterCommand(
    ...
    function()
        if globalOnPoliceDuty or globalNHSOnDuty or globalOnPrisonDuty or globalLFBOnDuty or PlayerIsInGang then
            if b.Cooling == 0 then
                local f = {}
                f.Player = CORRUPT.getPlayerId()
                f.Ped = CORRUPT.getPlayerPed()
                f.Name = CORRUPT.getPlayerName(f.Player)
                ...
                if globalOnPoliceDuty then
                    TriggerServerEvent("CORRUPT:newPanic", f, 3)
                elseif globalNHSOnDuty then
                    TriggerServerEvent("CORRUPT:newPanic", f, 2)
                elseif globalOnPrisonDuty then
                    TriggerServerEvent("CORRUPT:newPanic", f, 4)
                elseif globalLFBOnDuty then
                    TriggerServerEvent("CORRUPT:newPanic", f, 5)
                elseif PlayerIsInGang then
                    TriggerServerEvent("CORRUPT:newGangPanic", f, 6)
                end
...
RegisterNetEvent(
    "CORRUPT:returnPanic",
    function(g, f, h)
        local i = CORRUPT.getUserId()
        if
            globalOnPoliceDuty or globalNHSOnDuty or globalOnPrisonDuty or globalLFBOnDuty and h == 5 or
                h == 6 and not CORRUPT.isEmergencyService()
         then
            if h == 6 and CORRUPT.isEmergencyService() then
                return
            end
