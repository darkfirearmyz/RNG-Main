        ...
        if t then
            ...
            ontick_prisonControlPanel = function()
                ...
                elseif IsControlJustPressed(0, 168) then
                    if CORRUPT.isDeveloper() and globalOnPrisonDuty then
                        d = true
                        setCursor(1)
                        inGUICORRUPT = true
                    end
    ...
    if d then
            ...
            if j[r].name == "All" then
                ...
                if not CursorInArea(GetArea(0.497, 0.819, 0.185, 0.058)) then
                    ...
                    if f then
                        if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                            if h[j[r].doorHash].currentState == 5 or h[j[r].doorHash].currentState == 0 then
                                TriggerServerEvent("CORRUPT:prisonSetDoorState", j[r].doorHash, 1)
                            else
                                TriggerServerEvent("CORRUPT:prisonSetDoorState", j[r].doorHash, 5)
                            end
                            PlaySound(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 0, 0, 1)
                            f = false
                            SetTimeout(
        ...
        if not CursorInArea(GetArea(0.791, 0.819, 0.081, 0.06)) then
            ...
            if e then
                if IsControlJustPressed(1, 329) or IsDisabledControlJustPressed(1, 329) then
                    c = not c
                    TriggerServerEvent("CORRUPT:prisonToggleLockdown", c)
                end
...
CORRUPT.createThreadOnTick(func_drawControlPanel)
RegisterNetEvent(
    "CORRUPT:prisonUpdateGuardNumber",
    function(v)
        g = v
    end
)
    ...
    function(s, t)
        if t then
            ...
            ontick_openPrisonDoor = function(w)
                ...
                if g > 4 then
                    if
                        tvRP.isStaffedOn() or globalOnPrisonDuty and w.overrideLock or CORRUPT.isDeveloper() or globalOnPoliceDuty and w.pdOpen or
                    ...
                    if
                        tvRP.isStaffedOn() or globalOnPrisonDuty or CORRUPT.isDeveloper() or globalOnPoliceDuty and w.pdOpen or
                            (globalNHSOnDuty or globalLFBOnDuty) and w.nhsOpen
                     then
                        x = " (E to toggle lock) "
                        y = true
                ...
                if h[w.doorHash].currentState == 5 or h[w.doorHash].currentState == 0 then
                    CORRUPT.DrawText3D(w.coords, w.doorName .. x .. " 🔓", 0.45, 4)
                else
                    CORRUPT.DrawText3D(w.coords, w.doorName .. x .. " 🔒", 0.45, 4)
                end
                if
                    IsControlJustPressed(0, 38) and
                        (tvRP.isStaffedOn() or globalOnPrisonDuty and (y or w.overrideLock) or CORRUPT.isDeveloper() or
                            globalOnPoliceDuty and w.pdOpen or
                            (globalNHSOnDuty or globalLFBOnDuty) and w.nhsOpen)
                 then
                    -- CORRUPT.loadAnimDict("anim@heists@keycard@")
                    -- Citizen.CreateThread(
                    --     function()
                    ...
                    if h[w.doorHash].currentState == 1 then
                        TriggerServerEvent("CORRUPT:prisonSetDoorState", w.doorHash, 5)
                    else
                        TriggerServerEvent("CORRUPT:prisonSetDoorState", w.doorHash, 1)
                    end
                end
            end
            for k, l in pairs(a.doors) do
...
RegisterNetEvent(
    "CORRUPT:prisonSetAllDoorStates",
    function(B)
        if B ~= nil then
