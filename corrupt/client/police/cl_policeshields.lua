        ...
        while true do
            if a.active and a.id ~= 0 then
                ...
                if e ~= `WEAPON_UNARMED` and (not b[a.id].allowPistol or not table.has(c, e)) then
                    CORRUPT.setWeapon(d, "WEAPON_UNARMED", true)
                end
...
function removeShield(l)
    if DoesEntityExist(a.prop) then
        ...
        if l then
            tvRP.notify("~g~Success~w~: Shield removed")
        end
    end
end
function createShield(m)
    ...
    a.id = m
    local p = CORRUPT.loadModel(b[m].model)
    ...
    CORRUPT.loadAnimDict(b[m].animDict)
