...
AddEventHandler(
    ...
    function(m, n)
        if n then
            ...
            ontick_openPoliceDoor = function(o)
                ...
                if IsControlJustPressed(0, 38) and globalOnPoliceDuty then
                    ...
                    if a[o.doorHash] == 4 then
                        TriggerServerEvent("CORRUPT:policeSetDoorState", o.doorHash, 5)
                    else
                        TriggerServerEvent("CORRUPT:policeSetDoorState", o.doorHash, 4)
                    end
                end
            end
            for q = 1, #b, 1 do
            ...
            local r = function()
                TriggerServerEvent("CORRUPT:enterPoliceSyncDoors")
            end
            for q = 1, #c do
                CORRUPT.createArea(
