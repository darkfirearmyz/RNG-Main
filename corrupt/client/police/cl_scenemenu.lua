...
RMenu.Add(
    ...
    RageUI.CreateSubMenu(
        RMenu:Get("corruptscenemenu", "main"),
        "",
        "~b~Spawn Objects",
        CORRUPT.getRageUIMenuWidth(),
        CORRUPT.getRageUIMenuHeight(),
        ...
        RageUI.IsVisible(
            ...
            function()
                ...
                RageUI.ButtonWithStyle(
                    ...
                    RMenu:Get("corruptscenemenu", "objects")
                )
                RageUI.ButtonWithStyle(
                    "Speed Zone",
                    nil,
                    ...
                    RMenu:Get("corruptscenemenu", "speedZone")
                )
                if CORRUPT.globalOnPoliceDuty() then
                    RageUI.ButtonWithStyle(
                        ...
                        RMenu:Get("corruptscenemenu", "spikes")
                    )
                end
                RageUI.ButtonWithStyle(
                    "Close Menu",
                    ...
                    function(J, K, L)
                        if L then
                            RageUI.Visible(RMenu:Get("corruptscenemenu", "main"), false)
                        end
                    end
                )
            end,
            ...
            function()
                ...
                RageUI.ButtonWithStyle(
                    ...
                    function(J, K, L)
                        if L then
                            if o == 0 then
                                tvRP.notify("Please set a radius")
                            elseif p == 0 then
                                tvRP.notify("Please set a speed")
                            else
                                TriggerServerEvent("CORRUPT:createSpeedZone", CORRUPT.getPlayerCoords(), p + 0.0, o + 0.0)
                                tvRP.notify("~b~Speed zone created")
                            end
                        end
                    end,
                    nil
                ...
                RageUI.ButtonWithStyle(
                    ...
                    function(J, K, L)
                        if L then
                            ...
                            if Q then
                                TriggerServerEvent("CORRUPT:deleteSpeedZone", Q)
                                tvRP.notify("Speed zone removed")
                            end
                        end
                    end,
                    nil
...
RegisterNetEvent(
    "CORRUPT:toggleTrafficMenu",
    function()
        RageUI.Visible(RMenu:Get("corruptscenemenu", "main"), not RageUI.Visible(RMenu:Get("corruptscenemenu", "main")))
        if globalOnPoliceDuty and CORRUPT.hasPoliceCallsign() then
            RMenu:Get("corruptscenemenu", "main"):SetSubtitle(
                "~b~MPD~w~: " ..
                    tvRP.getPoliceCallsign() .. " - " .. tvRP.getPoliceRank() .. " - " .. CORRUPT.getPoliceName()
            )
        elseif CORRUPT.isDeveloper() then
            RMenu:Get("corruptscenemenu", "main"):SetSubtitle("Dev")
        end
    end
)
function deleteObject()
    for m, n in pairs(j) do
        ...
        local X, Y, Z = table.unpack(GetEntityCoords(CORRUPT.getPlayerPed(), true))
        if DoesObjectOfTypeExistAtCoords(X, Y, Z, 0.9, W, true) then
            ...
            if NetworkGetEntityIsNetworked(_) and not NetworkHasControlOfEntity(_) then
                local a0 = CORRUPT.getNetId(_, "deleteObject()")
                if a0 ~= 0 then
                    TriggerServerEvent("CORRUPT:requestSceneObjectDelete", a0)
                end
            else
                DeleteObject(_)
            end
            ...
            for m, a1 in pairs(a) do
                if DoesEntityExist(a1) then
                    if NetworkGetEntityIsNetworked(a1) then
                        ...
                        if a2 ~= 0 then
                            TriggerServerEvent("CORRUPT:requestSceneObjectDelete", a2)
                        end
                    else
                        DeleteEntity(a1)
                    end
...
function placeSpike()
    local a3 = CORRUPT.getPlayerVehicle()
    if a3 == 0 then
        local a4 = "P_ld_stinger_s"
        local a5 = CORRUPT.getPlayerPed()
        ...
        TriggerServerEvent("CORRUPT:placeSpike", a6, P)
    end
end
...
function deleteSpike()
    ...
    if a7 then
        TriggerServerEvent("CORRUPT:removeSpike", a7[2])
    end
end
RegisterNetEvent(
    "CORRUPT:addSpike",
...
Citizen.CreateThread(
    function()
        while true do
            local a3, af = CORRUPT.getPlayerVehicle()
            if a3 ~= 0 and af then
                local ag = false
                local ah = 1000.0
