...
RegisterNetEvent(
    "CORRUPT:flashbangExplode",
    function(coords)
        if #(coords - CORRUPT.getPlayerCoords()) <= b then
...
function func_checkForFlashbang()
    if not a then
        if GetSelectedPedWeapon(CORRUPT.getPlayerPed()) == `WEAPON_FLASHBANG` then
            a = true
        else
            a = false
        end
    else
        if IsPedShooting(CORRUPT.getPlayerPed()) and not c then
            c = true
            Citizen.CreateThread(
                function()
                    ...
                    if e ~= 0 then
                        ...
                        AddExplosion(coords, 25, 0.0, 1.0, nil, true, true)
                        TriggerServerEvent("CORRUPT:flashbangThrown", f)
                    end
                end
            )
            SetTimeout(
...
CORRUPT.createThreadOnTick(func_checkForFlashbang)
