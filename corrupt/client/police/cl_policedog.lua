RMenu.Add(
    "policedog",
    "main",
    RageUI.CreateMenu("Dog Support Unit", "Dog Management", CORRUPT.getRageUIMenuWidth(), CORRUPT.getRageUIMenuHeight())
)
AddEventHandler(
...
local j = "K9TARGET"
RegisterNetEvent(
    "CORRUPT:policeDogMenu",
    function()
        if globalOnPoliceDuty and CORRUPT.hasPoliceCallsign() then
            RMenu:Get("frscenemenu", "main"):SetSubtitle(
                "MPD~w~: " ..
                    tvRP.getPoliceCallsign() ..
                        " - " .. tvRP.getPoliceRank() .. " - " .. CORRUPT.getPoliceName()
            )
        end
        if globalOnPrisonDuty and CORRUPT.hasHmpCallsign() then
            RMenu:Get("frscenemenu", "main"):SetSubtitle(
                "HMP~w~: " .. CORRUPT.getHmpCallsign() .. " - " .. CORRUPT.getHmpRank() .. " - " .. CORRUPT.getHmpName()
            )
        end
...
RegisterNetEvent(
    "CORRUPT:sendClientRagdoll",
    function()
        SetPedToRagdoll(CORRUPT.getPlayerPed(), 12000, 12000, 0, 0, 0, 0)
        showNotification("~y~~h~Alert~h~~s~: " .. "You have been bitten by a police dog.")
    end
)
RegisterNetEvent(
    "CORRUPT:policeDogAttack",
    function()
...
local m = {breed = 1}
local function n()
    local o = GetResourceKvpString("corrupt_policedog_customisations")
...
local function r(s)
    ...
    local q = k[m.breed]
    p[q] = s
    SetResourceKvp("corrupt_policedog_customisations", json.encode(p))
end
RageUI.CreateWhile(
    1.0,
    RMenu:Get("policedog", "main"),
    nil,
    function()
        RageUI.IsVisible(
            RMenu:Get("policedog", "main"),
...
function tpDog()
    local z = GetEntityCoords(CORRUPT.getPlayerPed(), true)
                                        ...
                                        if M < 2.0 then
                                            ...
                                            for O, P in pairs(N) do
                                                if I == GetPlayerPed(P) then
                                                    ...
                                                    TriggerServerEvent(
                                                        "CORRUPT:performDogLog",
                                                        "Instructed dog to attack ID: " .. Q
                                                    )
                                                    TriggerServerEvent("CORRUPT:serverDogAttack", Q)
                                                    H = true
                                                    Wait(1000)
    ...
    while f.currentAction == d.SearchNearby and DoesEntityExist(f.handle) do
        ...
        for W, X in pairs(GetActivePlayers()) do
            ...
            if GetVehiclePedIsUsing(Y) == 0 and IsEntityVisible(Y) and #(GetEntityCoords(Y, true) - L) < 5.0 then
                ...
                local _ = CORRUPT.clientGetUserIdFromSource(Z)
                if CORRUPT.getJobType(_) == "" then
        ...
        if #V > 0 then
            ...
            TriggerServerEvent("CORRUPT:policeDogSniffPlayer", a0.source)
        end
        Citizen.Wait(5000)
    end
end
function putInVehicle()
    local a2 = CORRUPT.getNearestVehicle(7.0)
    Citizen.Trace(a2)
...
function removeFromVehicle()
    if IsPedInAnyVehicle(CORRUPT.getPlayerPed(), true) then
...
function newDog(q, a4)
    ...
    if not IsPedInAnyPoliceVehicle(a5) then
        ...
        if DoesEntityExist(f.handle) then
            ...
            TriggerServerEvent("CORRUPT:performDogLog", "Created a new police dog")
        end
    end
end
function deleteDog()
    ...
    f.active = false
    f.handle = 0
    TriggerServerEvent("CORRUPT:performDogLog", "Deleted dog")
end
function dogFollow()
    if not f.active then
        return
    ...
    if f.currentAction ~= d.Follow then
        ClearPedTasks(f.handle)
        TaskFollowToOffsetOfEntity(f.handle, CORRUPT.getPlayerPed(), 0.0, 0.0, 0.0, 7.0, -1, 10.0, true)
        f.currentAction = d.Follow
    else
        ClearPedTasks(f.handle)
        f.currentAction = d.Follow
        ...
        while f.currentAction == d.Indicate and DoesEntityExist(af) do
            ...
            elseif GetGameTimer() - ag > 3500 then
                if not IsEntityPlayingAnim(f.handle, g.Sit.dict, g.Sit.anims.base, 3) then
                    CORRUPT.loadAnimDict(g.Sit.dict)
