...
RMenu.Add(
    "corruptpdelevators",
    "main",
    RageUI.CreateMenu("CORRUPT Elevators", "Vespucci PD", CORRUPT.getRageUIMenuWidth(), CORRUPT.getRageUIMenuHeight())
)
RageUI.CreateWhile(
    1.0,
    RMenu:Get("corruptpdelevators", "main"),
    nil,
    function()
        RageUI.IsVisible(
            RMenu:Get("corruptpdelevators", "main"),
...
AddEventHandler(
    ...
    function(i, j)
        if j then
            local k = function()
                RageUI.Visible(RMenu:Get("corruptpdelevators", "main"), true)
            end
            local l = function()
                RageUI.Visible(RMenu:Get("corruptpdelevators", "main"), false)
            end
            local m = function()
                RMenu:Get("corruptpdelevators", "main"):SetSubtitle(string.format("~w~You are on ~b~%s", a[b].name))
            end
            for n, o in pairs(a) do
                CORRUPT.createArea("pdelevator_" .. n, o.coords, 1.5, 6, k, l, m)
                tvRP.addMarker(
                    o.coords.x,
                    o.coords.y,
                    o.coords.z - 0.98,
