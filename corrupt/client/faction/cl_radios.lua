...
local b = {}
local c = 0
function CORRUPT.isInRadioChannel()
    return c ~= 0
end
local function d()
...
RMenu.Add(
    "radios",
    "mainmenu",
    RageUI.CreateMenu("Radios", "Main Menu", CORRUPT.getRageUIMenuWidth(), CORRUPT.getRageUIMenuHeight())
)
RMenu.Add(
    "radios",
    "advancedeffects",
...
RegisterCommand(
    ...
    function()
        ...
        TriggerEvent("CORRUPT:showRadioWheel", X)
    end,
    false
)
...
Citizen.CreateThread(
    function()
        ...
        exports["pma-voice"]:setRadioVolume(u("radios_volume", 10) * 10)
        B = CreateAudioSubmix("CORRUPTRadio")
...
RegisterCommand(
    ...
    function()
        if c ~= 0 then
            if exports["pma-voice"]:getRadioVolume() == 0 then
                exports["pma-voice"]:setRadioVolume(u("radios_volume", 10) * 10)
                TriggerServerEvent("CORRUPT:radiosSetIsMuted", false)
            else
                exports["pma-voice"]:setRadioVolume(0)
                TriggerServerEvent("CORRUPT:radiosSetIsMuted", true)
            end
        end
    end,
    false
...
RegisterNetEvent(
    "CORRUPT:radiosSetPlayerIsMuted",
    function(q, j, p)
        local s = b[q]
        if not s then
            return
...
RegisterNetEvent(
    "CORRUPT:clientJoinRadioChannel",
    function(q)
        if b[q] and c ~= q then
            J(q, b[q])
        end
...
RegisterNetEvent(
    "CORRUPT:clientLeaveRadioChannel",
    function()
        if c ~= 0 then
            t()
        end
...
AddEventHandler(
    "CORRUPT:openRadioConfig",
    function()
        RageUI.Visible(RMenu:Get("radios", "mainmenu"), true)
    end
)
function CORRUPT.radioUIVisible(Y)
    e(Y)
end
