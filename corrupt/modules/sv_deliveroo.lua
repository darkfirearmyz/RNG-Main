...
RegisterNetEvent("CORRUPT:attemptBeginDeliverooJob")
AddEventHandler("CORRUPT:attemptBeginDeliverooJob",function()
    local source=source
    local user_id=CORRUPT.getUserId(source)
    local deliverooJobTable={}
    if CORRUPT.hasGroup(user_id,"Deliveroo")then
        if not usersInDeliverooJob[user_id] then
            ...
            TriggerClientEvent("CORRUPT:beginDeliverooJob",source)
            Wait(100)
            TriggerClientEvent("CORRUPT:deliverooJobSetNextBlip",source,deliverooStops[deliverooJobTable.stopNumber].stopPosition)
        else
            CORRUPT.notify(source,{"You are already a Deliveroo Driver."})
        end
    else
        CORRUPT.notify(source,{"~r~You are not clocked on as a Deliveroo Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(deliverooStops)do
                    if CORRUPT.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(CORRUPT.getUserSource(k)))-deliverooStops[v.currentJob.stopNumber].stopPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            CORRUPT.giveBankMoney(k,pay)
                            TriggerClientEvent("CORRUPT:deliverooJobReachedNextStop",CORRUPT.getUserSource(k),pay)
                            if v.currentJob.stopNumber==#deliverooStops then
                                TriggerClientEvent("CORRUPT:deliverooJobEnd", CORRUPT.getUserSource(k), true)
                                v.currentJob.jobActive=false
                                usersInDeliverooJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("CORRUPT:deliverooJobSetNextBlip",CORRUPT.getUserSource(k),deliverooStops[v.currentJob.stopNumber].stopPosition)
                            end
                        end
                    end
                end
