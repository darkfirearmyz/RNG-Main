...
function CORRUPT.openMenu(source,menudef)
  local menudata = {}
  menudata.choices = {}
  ...
  -- openmenu
  CORRUPTclient.openMenuData(source,{menudata})
end

-- force close player menu
function CORRUPT.closeMenu(source)
  CORRUPTclient.closeMenu(source,{})
end
...
function CORRUPT.prompt(source,title,default_text,cb_result)
  prompts[source] = cb_result

  CORRUPTclient.prompt(source,{title,default_text})
end
...
function CORRUPT.request(source,text,time,cb_ok)
  local id = request_ids:gen()
  local request = {source = source, cb_ok = cb_ok, done = false}
  requests[id] = request

  CORRUPTclient.request(source,{id,text,time}) -- send request to client
...
function CORRUPT.registerMenuBuilder(name, builder)
...
function CORRUPT.buildMenu(name, data, cbr)
...
function CORRUPT.openMainMenu(source)
  CORRUPT.buildMenu("main", {player = source}, function(menudata)
    menudata.name = "Main menu"
    menudata.css = {top="75px",header_color="rgba(0,125,255,0.75)"}
    CORRUPT.openMenu(source,menudata) -- open the generated menu
  end)
end
...
function CORRUPT.validMenuChoice(id,choice,mod)
  local menu = client_menus[id]
  if menu and menu.source == source then
...
RegisterServerEvent("CORRUPT:promptResult", function(result)
  local source = source
  if result == nil then
    result = ""
  end
...
RegisterServerEvent("CORRUPT:requestResult", function(id,ok)
  local source = source
...
function CORRUPT.addStaticMenuChoices(name, choices)
...
-- build static menus
local static_menus = {}
SetTimeout(10000,function() -- wait for CORRUPT.addStaticMenuChoices calls
...
AddEventHandler("CORRUPT:playerSpawn",function(user_id, source, first_spawn)
  if first_spawn then
    ...
    CORRUPTclient.setDiv(source,{"additional_css",".div_additional_css{ display: none; }\n\n"..cfg.css,""})
