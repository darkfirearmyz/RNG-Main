...
RegisterNetEvent("CORRUPT:toggleShiftG4S")
AddEventHandler("CORRUPT:toggleShiftG4S", function(Status)
    local source=source
    local user_id=CORRUPT.getUserId(source)
    if CORRUPT.hasGroup(user_id,"G4S Driver")then
        if Status then
            currentUsersOnG4SJob[user_id]={currentJob=""}
            TriggerClientEvent("CORRUPT:startShiftG4S",source,math.random(1000,9999))
            jobsCompleted[user_id] = 0
            Wait(10000)
            TriggerClientEvent("CORRUPT:updateJobInformation",source,GetCollection(user_id))
        else
            if currentUsersOnG4SJob[user_id]~=nil then
                currentUsersOnG4SJob[user_id]=nil
                jobsCompleted[user_id] = nil
                TriggerClientEvent("CORRUPT:endShiftG4S",source)
            end
        end
    else
        CORRUPT.notify(source,{"~r~You are not clocked on as currentUsersOnG4SJob G4S Driver."})
    end
end)

RegisterNetEvent("CORRUPT:acceptJob")
AddEventHandler("CORRUPT:acceptJob",function()
    local source=source
    local user_id=CORRUPT.getUserId(source)
    if CORRUPT.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
            if k == user_id then    
                ...
                TriggerClientEvent("CORRUPT:updateJobInformation",source,v.currentJob)
            end
        end
    else
        CORRUPT.notify(source,{"~r~You are not clocked on as G4S Driver."})
    end
end)

RegisterNetEvent("CORRUPT:updateMoneyInVehicle")
AddEventHandler("CORRUPT:updateMoneyInVehicle", function()
    local source=source
    local user_id=CORRUPT.getUserId(source)
    if CORRUPT.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
...
RegisterNetEvent("CORRUPT:updateMoneyOutVehicle")
AddEventHandler("CORRUPT:updateMoneyOutVehicle", function()
    local source=source
    local user_id=CORRUPT.getUserId(source)
    if CORRUPT.hasGroup(user_id,"G4S Driver")then
        for k,v in pairs(currentUsersOnG4SJob)do
            ...
            if v.currentJob.jobActive then
                if CORRUPT.getUserSource(k)then
                    if not v.currentJob.collected then
                        if #(GetEntityCoords(GetPlayerPed(CORRUPT.getUserSource(k)))-v.currentJob.collectionCoords)<1.5 then
                            v.currentJob.collected=true
                            currentUsersOnG4SJob[k].currentJob=v.currentJob
                            TriggerClientEvent("CORRUPT:updateJobInformation",CORRUPT.getUserSource(k),v.currentJob)
                            TriggerClientEvent("CORRUPT:requestMoneyInVehicle",CORRUPT.getUserSource(k))
                        end
                    elseif v.currentJob.moneyInVehicle and not v.currentJob.depositing and not v.currentJob.moneyOutVehicle then
                        if #(GetEntityCoords(GetPlayerPed(CORRUPT.getUserSource(k)))-v.currentJob.depositCoords)<20 then
                            v.currentJob.depositing=true
                            currentUsersOnG4SJob[k].currentJob=v.currentJob
                            TriggerClientEvent("CORRUPT:updateJobInformation",CORRUPT.getUserSource(k),v.currentJob)
                            TriggerClientEvent("CORRUPT:requestMoneyOutVehicle",CORRUPT.getUserSource(k))
                        end
                    elseif not v.currentJob.deposited and v.currentJob.moneyOutVehicle then
                        if #(GetEntityCoords(GetPlayerPed(CORRUPT.getUserSource(k)))-v.currentJob.depositCoords)<1.5 then
                            v.currentJob.deposited=true
                            v.currentJob.depositing=false
                            TriggerClientEvent("CORRUPT:updateJobInformation",CORRUPT.getUserSource(k),GetCollection(k))
...
RegisterCommand('g4s', function(source, args)
    ...
    for k,v in pairs(currentUsersOnG4SJob) do
        if k == user_id then
            TriggerClientEvent("CORRUPT:openG4SMenu", source)
        end
    end
end)