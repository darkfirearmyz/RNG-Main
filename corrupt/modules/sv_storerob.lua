...
RegisterNetEvent('CORRUPT:getStoreRobBlips')
AddEventHandler('CORRUPT:getStoreRobBlips', function()
    TriggerClientEvent('CORRUPT:updateStoreRobBlips', source, stores)
end)


RegisterNetEvent('CORRUPT:initiateStoreRobbery')
AddEventHandler('CORRUPT:initiateStoreRobbery', function(store)
    local source = source
    local user_id = CORRUPT.getUserId(source)
    if CORRUPT.hasPermission(user_id, "police.armoury") then
        TriggerClientEvent('CORRUPT:resetStorePed', source, store)
    else
        if #CORRUPT.getUsersByPermission('police.armoury') > 0 then
            for k,v in pairs(stores) do
                if k == store then
                    if v.cooldown == 0 then
                        v.beingrobbed = true
                        v.cooldown = 300
                        TriggerClientEvent('CORRUPT:updateStoreRobBlips', -1, stores)
                        TriggerClientEvent('CORRUPT:beginStoreRobbingAnimations', -1, store)
                        TriggerClientEvent('CORRUPT:storeRobberyInProgress', source, true, store)
                        ...
                        TriggerEvent('CORRUPT:PDRobberyCall', source, v.storename, v.position)
                    else
            ...
            CORRUPT.notify(source, {'~r~There are not enough police on duty to rob a store.'})
            TriggerClientEvent('CORRUPT:resetStorePed', source, store)
        end
    end
end)

RegisterNetEvent('CORRUPT:completeSafeCracking')
AddEventHandler('CORRUPT:completeSafeCracking', function(store)
    local source = source
    local user_id = CORRUPT.getUserId(source)
    local storebeingrobbed = false
    for k,v in pairs(stores) do
        if k == store then
            if v.beingrobbed then
                storebeingrobbed = true
            end
        end
    end
    if storebeingrobbed then
        CORRUPT.giveDirtyMoney(user_id, math.random(100000, 500000))
        TriggerClientEvent('CORRUPT:syncCloseSafeDoor', -1, store)
        TriggerClientEvent('CORRUPT:resetStorePed', -1, store)
        for k,v in pairs(stores) do
            if k == store then
                v.beingrobbed = false
            end
        ...
        TriggerClientEvent('CORRUPT:updateStoreRobBlips', -1, stores)
        TriggerClientEvent('CORRUPT:storeRobberyInProgress', source, false, store)
    else
        CORRUPT.notify(source, {'~r~The store is no longer being robbed.'})
    end
end)

RegisterNetEvent('CORRUPT:forceEndRobbery')
AddEventHandler('CORRUPT:forceEndRobbery', function(store)
    local source = source
    local user_id = CORRUPT.getUserId(source)
    TriggerClientEvent('CORRUPT:resetStorePed', -1, store)
    for k,v in pairs(stores) do
...
RegisterNetEvent('CORRUPT:syncOpenSafeDoor')
AddEventHandler('CORRUPT:syncOpenSafeDoor', function(store)
    local source = source
    TriggerClientEvent('CORRUPT:syncOpenSafeDoor', -1, store)
end)
