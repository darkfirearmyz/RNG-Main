...
RegisterNetEvent("CORRUPT:attemptBeginRoyalMailJob")
AddEventHandler("CORRUPT:attemptBeginRoyalMailJob",function()
    local source=source
    local user_id=CORRUPT.getUserId(source)
    local RoyalMailJobTable={}
    if CORRUPT.hasGroup(user_id,"Royal Mail Driver")then
        if not usersInRoyalMailJob[user_id] then
            ...
            TriggerClientEvent("CORRUPT:beginRoyalMailJob",source)
            Wait(100)
            TriggerClientEvent("CORRUPT:royalMailJobSetNextBlip",source,RoyalMailDrops[RoyalMailJobTable.stopNumber].dropPosition)
        else
            CORRUPT.notify(source,{"You are already a Royal Mail Driver."})
        end
    else
        CORRUPT.notify(source,{"~r~You are not clocked on as a Royal Mail Driver."})
    end
end)
            ...
            if v.currentJob.jobActive then
                for a,b in pairs(RoyalMailDrops)do
                    if CORRUPT.getUserSource(k)then
                        if#(GetEntityCoords(GetPlayerPed(CORRUPT.getUserSource(k)))-RoyalMailDrops[v.currentJob.stopNumber].dropPosition)<5.0 then
                            local pay = grindBoost*math.random(1000,1500)
                            CORRUPT.giveBankMoney(k,pay)
                            TriggerClientEvent("CORRUPT:royalMailReachedNextStop",CORRUPT.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition,pay)
                            if v.currentJob.stopNumber==#RoyalMailDrops then
                                TriggerClientEvent("CORRUPT:royalMailJobEnd", CORRUPT.getUserSource(k))
                                v.currentJob.jobActive=false
                                usersInRoyalMailJob[user_id] = nil
                            else
                                v.currentJob.stopNumber=v.currentJob.stopNumber+1
                                TriggerClientEvent("CORRUPT:royalMailJobSetNextBlip",CORRUPT.getUserSource(k),RoyalMailDrops[v.currentJob.stopNumber].dropPosition)
                            end
                        end
                    end
                end
