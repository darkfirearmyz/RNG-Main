...
function CORRUPT.getUserGroups(user_id)
...
function CORRUPT.addUserGroup(user_id, group)
    if not CORRUPT.hasGroup(user_id, group) then
        local user_groups = CORRUPT.getUserGroups(user_id)
        ...
        if ngroup then
            ...
            TriggerEvent("CORRUPT:playerJoinGroup", user_id, group, gtype)
        end
    end
end
...
function CORRUPT.getUserGroupByType(user_id, gtype)
    local user_groups = CORRUPT.getUserGroups(user_id)
    for k, v in pairs(user_groups) do
        local kgroup = groups[k]
        if kgroup then
...
function CORRUPT.getUsersByGroup(group)
    local users = {}

    for k, v in pairs(CORRUPT.rusers) do
        if CORRUPT.hasGroup(tonumber(k), group) then
...
function CORRUPT.getUsersByPermission(perm)
    local users = {}

    for k, v in pairs(CORRUPT.rusers) do
        if CORRUPT.hasPermission(tonumber(k), perm) then
            table.insert(users, tonumber(k))
        end
    end
...
function CORRUPT.removeUserGroup(user_id, group)
    local user_groups = CORRUPT.getUserGroups(user_id)
    ...
    TriggerEvent("CORRUPT:playerLeaveGroup", user_id, group, gtype)

    user_groups[group] = nil -- remove reference
end

-- check if the user has a specific group
function CORRUPT.hasGroup(user_id, group)
    local user_groups = CORRUPT.getUserGroups(user_id)
...
-- check if the user has a specific permission
function CORRUPT.hasPermission(user_id, perm)
    local user_groups = CORRUPT.getUserGroups(user_id)
    ...
    if fchar == "@" then -- special aptitude permission
        ...
        if #parts == 3 then -- decompose group.aptitude.operator
            ...
            local afr = math.floor(CORRUPT.expToLevel(CORRUPT.getExp(user_id, group, aptitude)))

            local fop = string.sub(op, 1, 1)
            if fop == "<" then -- less (group.aptitude.<x)
                local corrupt = parseInt(string.sub(op, 2, string.len(op)))
                if afr < corrupt then
                    return true
                end
            elseif fop == ">" then -- greater (group.aptitude.>x)
                ...
                local corrupt = parseInt(string.sub(op, 1, string.len(op)))
                if afr == corrupt then
                    return true
                end
            end
    ...
    elseif fchar == "#" then -- special item permission
        ...
        if #parts == 2 then -- decompose item.operator
            ...
            local amount = CORRUPT.getInventoryItemAmount(user_id, item)
...
function CORRUPT.hasPermissions(user_id, perms)
...
exports('hasPermission', function(erm, table)
    local source = erm.source
    local perm = erm.perm
    local functionevent = CORRUPT.hasPermission(CORRUPT.getUserId(source), perm)
    return functionevent
end)

exports('hasGroup', function(source, group)
    return CORRUPT.hasGroup(CORRUPT.getUserId(source), group)
end)