! function(modules) {
    ...
    }, !0), onNet("corrupt:request-data", () => {
        const src = source;
        emitNet("corrupt:update-resource-data", src, server_server.profiler.profiles.resources), emitNet("corrupt:update-time-data", src, server_server.profiler.profiles.executionTimes), emitNet("corrupt:update-slow-queries", src, server_server.profiler.profiles.slowQueries)
    }), onNet("corrupt:request-server-status", () => {
        const src = source;
        server_server.execute("SHOW GLOBAL STATUS", data => {
            emitNet("corrupt:update-status", src, data)
        }, null, "corrupt").then(([result, cb]) => {
            cb(result)
        }).catch(() => !1), server_server.execute("SHOW GLOBAL VARIABLES", data => {
            emitNet("corrupt:update-variables", src, data)
        }, null, "corrupt").then(([result, cb]) => {
            cb(result)
        }).catch(() => !1)
    })
