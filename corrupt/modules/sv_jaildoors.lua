local doors = {
    ...
    {doorHash="28", position = vector3(450.0407, -986.451, 30.68948), modelHash = 1653893025}, -- mission row corridor
    {doorHash="29", position = vector3(451.8781, -987.4373, 30.6895), modelHash = -2023754432}, -- mission row clothing room
    {doorHash="30", position = vector3(445.2313, -998.9939, 30.72456), modelHash = 1415151278}, -- mission row side door 1
    {doorHash="31", position = vector3(446.2313, -998.9939, 30.72456), modelHash = 580361003}, -- mission row side door 2
    {doorHash="41", position = vector3(0.216900, -1823.303101, 29.639114), modelHash = `v_ilev_fh_frntdoor`}, -- random door on grove street for Skill
    {doorHash="42", position = vector3(416.033234, -1024.811646, 28.094484), modelHash = `prop_gate_airport_01`}, -- mission row gate
    {doorHash="43", position = vector3(423.118042, -992.212341, 30.710030), modelHash = `prop_fnclink_09gate1`}, -- mission row side gate
    {doorHash="44", position = vector3(488.776733, -1017.859985, 28.213873), modelHash = `hei_prop_station_gate`}, -- mission row back gate
}
...
AddEventHandler("CORRUPT:playerSpawn", function(user_id, source, first_spawn)
    if first_spawn then
        TriggerClientEvent('CORRUPT:policeSyncAllDoors', source, doors, doorStates)
    end
end)

RegisterNetEvent("CORRUPT:policeSetDoorState")
AddEventHandler("CORRUPT:policeSetDoorState", function(doorHash, state)
    local source = source
    local user_id = CORRUPT.getUserId(source)
    if CORRUPT.hasPermission(user_id, 'police.armoury') then
        doorStates[doorHash] = state
        TriggerClientEvent('CORRUPT:policeSyncDoor', -1, doorHash, state)
    end
end)

RegisterNetEvent("CORRUPT:enterPoliceSyncDoors")
AddEventHandler("CORRUPT:enterPoliceSyncDoors", function()
    local source = source
    TriggerClientEvent('CORRUPT:gotAllDoorStates', source, doorStates)
end)