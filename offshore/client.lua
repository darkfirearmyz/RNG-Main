...
CreateThread(function ()
    ...
    RegisterNetEvent("CORRUPT:setDisplayOffshore", function(value)
    ...
    RegisterNUICallback("depositOffshoreMoney", function(data, cb)
        if data.moneyAmount then
            ...
            if moneyAmount then
                TriggerServerEvent("CORRUPT:depositOffshoreMoney", moneyAmount)
            end
        end
        cb("ok")
    end)
    ...
    RegisterNUICallback("depositAllOffshoreMoney", function(data, cb)
        TriggerServerEvent("CORRUPT:depositAllOffshoreMoney")
        cb("ok")
    end)
    ...
    RegisterNUICallback("withdrawOffshoreMoney", function(data, cb)
        if data.moneyAmount then
            local moneyAmount = tonumber(data.moneyAmount)
            if moneyAmount then
                TriggerServerEvent("CORRUPT:withdrawOffshoreMoney", moneyAmount)
            end
        end
        cb("ok")
    end)
    ...
    RegisterNUICallback("withdrawAllOffshoreMoney", function(data, cb)
        TriggerServerEvent("CORRUPT:withdrawAllOffshoreMoney")
        cb("ok")
    end)
